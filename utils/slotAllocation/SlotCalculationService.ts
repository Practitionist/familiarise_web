  ...
  static calculateRequiredSlots(
    ...
    switch (eventType) {
      ...
      case "subscription": {
        ...
        const totalWeeks = this.countWeeks(config.schedulingPeriodStartsAt, config.schedulingPeriodEndsAt);
  ...
  static calculateProgress(
    ...
    switch (eventType) {
      ...
      case "class": {
        ...
        if (!config.schedulingPeriodStartsAt || !config.schedulingPeriodEndsAt || !config.callsPerWeek) {
          throw new Error(
            "Start date, end date, and calls per week are required for subscription/class progress calculation",
          );
        }

        const weeks = this.countWeeks(config.schedulingPeriodStartsAt, config.schedulingPeriodEndsAt);
