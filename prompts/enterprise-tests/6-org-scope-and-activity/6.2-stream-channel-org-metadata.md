...
- `lib/meeting.ts` — `createMeeting`, `getOrCreateMeeting` (server-side; stamps `custom.organizationId`)
...
Capture `<learnpro-id>`, `<webinarId>`, `<meetingId>`.
...
mcp__supabase__execute_sql({
  ...
  query: `
    ...
    FROM "Meeting" ms
    JOIN "AppointmentOccurrence" soa ON soa.id = ms."slotOfAppointmentId"
