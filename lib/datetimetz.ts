...
export function convertSlotsToUserTimezone(
  slots: TSlotTiming[],
  userTimeZone: string,
): TSlotTiming[] {
  try {
    return slots.map((slot) => ({
      ...slot,
      dateInISO:
        convertUTCToZoneTime(slot.dateInISO, userTimeZone) || slot.dateInISO,
      localStartTime:
        convertUTCToZoneTime(slot.slotStartTimeInUTC, userTimeZone) ||
        slot.localStartTime,
      localEndTime:
        convertUTCToZoneTime(slot.slotEndTimeInUTC, userTimeZone) ||
        slot.localEndTime,
    }));
  } catch (error) {
