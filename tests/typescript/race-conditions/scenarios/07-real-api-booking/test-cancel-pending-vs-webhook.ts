...
async function run() {
  ...
  const appointment = await prisma.appointment.findFirst({
    where: {
      consultation: {
        requestStatus: { in: ["PENDING", "APPROVED", "APPROVED_PENDING_PAYMENT"] },
      },
  ...
  if (!appointment?.consultation?.requestedBy?.user?.email) {
    console.log("⏭️  SKIP — no consultation appointment with a requester in seed data");
    process.exit(0);
  }
  ...
  const leg2 = await createPendingPayment(requester.id, appointment.id, "leg2");
  ...
  await prisma.payment.deleteMany({
    where: {
      id: { in: [leg1?.paymentId, leg2.paymentId].filter(Boolean) as string[] },
    },
  });
  await prisma.consultation.update({
    where: { id: consultation.id },
