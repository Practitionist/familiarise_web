...
async function testBucketCreation(): Promise<void> {
  ...
  try {
    ...
    console.log(`   Result: ${anonResult.exists ? '✅ EXISTS' : '❌ NOT FOUND'}`);
    ...
    if (supabaseAdmin) {
      ...
      console.log(`   Result: ${adminResult.exists ? '✅ EXISTS' : '❌ NOT FOUND'}`);
...
async function testBucketExists(client: SupabaseClient, bucketName: string): Promise<BucketTestResult> {
  try {
