...
describe("validateWeeklySlotTimeOrder", () => {
  it("should accept valid same-day slot", () => {
    expect(
      validateWeeklySlotTimeOrder(DayOfWeek.MONDAY, DayOfWeek.MONDAY, 540, 1020),
    ).toBeNull();
  });

  it("should reject same-day slot where start >= end", () => {
    expect(
      validateWeeklySlotTimeOrder(DayOfWeek.MONDAY, DayOfWeek.MONDAY, 1020, 540),
    ).toContain("before end time");
  });

  it("should accept valid overnight slot (Mon 22:00 -> Tue 02:00)", () => {
    expect(
      validateWeeklySlotTimeOrder(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, 1320, 120),
    ).toBeNull();
  });

  it("should reject overnight slot spanning >1 day", () => {
    expect(
      validateWeeklySlotTimeOrder(DayOfWeek.MONDAY, DayOfWeek.WEDNESDAY, 1320, 120),
  ...
  it("should reject overnight slot that doesn't actually cross midnight", () => {
    ...
    expect(
      validateWeeklySlotTimeOrder(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, 120, 1320),
...
describe("slotsOverlap", () => {
  it("should detect overlap between same-day slots", () => {
    expect(
      slotsOverlap(
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.MONDAY, startTimeUtc: 540, endTimeUtc: 720 },
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.MONDAY, startTimeUtc: 600, endTimeUtc: 780 },
      ),
    ).toBe(true);
  });
  ...
  it("should not detect overlap for non-overlapping same-day slots", () => {
    expect(
      slotsOverlap(
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.MONDAY, startTimeUtc: 540, endTimeUtc: 600 },
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.MONDAY, startTimeUtc: 600, endTimeUtc: 720 },
      ),
    ).toBe(false);
  });
  ...
  it("should detect overlap between overnight slot and same-day slot on carry-over day", () => {
    ...
    expect(
      slotsOverlap(
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.TUESDAY, startTimeUtc: 1320, endTimeUtc: 120 },
        { startDay: DayOfWeek.TUESDAY, endDay: DayOfWeek.TUESDAY, startTimeUtc: 60, endTimeUtc: 120 },
      ),
    ).toBe(true);
  });
  ...
  it("should not overlap slots on different days", () => {
    expect(
      slotsOverlap(
        { startDay: DayOfWeek.MONDAY, endDay: DayOfWeek.MONDAY, startTimeUtc: 540, endTimeUtc: 720 },
        { startDay: DayOfWeek.TUESDAY, endDay: DayOfWeek.TUESDAY, startTimeUtc: 540, endTimeUtc: 720 },
      ),
    ).toBe(false);
  });
});
