...
The database was reset and reseeded (`small` mode). This guide gives you **every mock login** and a **click-through flow** to verify the enterprise subsystem across all org archetypes. Pair it with `ENTERPRISE_SUBSYSTEM_CHECKLIST.md` (what each phase covers + known gaps).
...
Each flow maps to phases in `ENTERPRISE_SUBSYSTEM_CHECKLIST.md`. Switch orgs via the org switcher (top-left) or the URL.
...
#    docs/enterprise/45-live-payout-go-live-runbook.md.
...
These are **known, tracked gaps** — verifying them as "not there" is correct. Full list + audit refs in `ENTERPRISE_SUBSYSTEM_CHECKLIST.md`. *(Updated 2026-06-05 post-v2 — several former gaps now ship; see the v2 flows above.)*
