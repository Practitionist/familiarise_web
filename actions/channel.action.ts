...
export async function initializeAllChannels() {
  ...
  try {
    ...
    console.log(`Successfully upserted ${uniqueUserIds.length} users to Stream Chat`);
  } catch (error) {
  ...
  for (const consultation of consultations) {
    try {
      ...
      console.error(`Error creating channel for consultation ${consultation.id}:`, error);
    }
  }
  ...
  for (const subscription of subscriptions) {
    try {
      ...
      console.error(`Error creating channel for subscription ${subscription.id}:`, error);
    }
  }
