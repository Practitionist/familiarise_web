...
export function V0ConsultationPricing({
  ...
  const subscriptionOptions: Option[] = React.useMemo(() => {
    return consultantDetails.subscriptionPlans
      ...
      .map((plan, index) => ({
        ...
        price: plan.price,
        badge:
          index === 1 ? "Most Popular" : index === 2 ? "Best Value" : undefined,
        unit: "/month" as const,
        plan,
      }));
  ...
  const [selected, setSelected] = React.useState<string>(
    consultationOptions.length > 1
      ? consultationOptions[1].value
      : consultationOptions[0]?.value || subscriptionOptions[0]?.value,
  );
  ...
  return (
    ...
    >
      <header className="text-center mb-8 md:mb-10">
        <div className="mb-3">
          <h1 className="text-pretty text-3xl md:text-4xl font-bold tracking-tight text-white">
      ...
      >
        <div className="grid grid-cols-2 gap-1.5">
          {consultationOptions.length > 0 && (
            <SegmentedTrigger
              isActive={plan === "consultation"}
              onClick={() => setPlan("consultation")}
            >
              Consultation
            </SegmentedTrigger>
          )}
        ...
        <div className="p-6 md:p-10 grid gap-8 md:gap-10">
          ...
          <div>
            ...
            <div className="flex items-end gap-2">
              ...
              <span className="text-gray-300 mb-2 text-lg">
                {activeOption?.unit}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2">
            ...
            <p className="text-sm md:text-base text-gray-400 sm:ml-2">
