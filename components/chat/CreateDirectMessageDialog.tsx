...
export const CreateDirectMessageDialog = ({
  ...
  const handleCreateDirectMessage = async () => {
    ...
    try {
      ...
      // Create a stable, unique channel ID using hash of full member IDs
      const sortedMembers = members.sort();
      
      // Create a stable hash from all member IDs (no truncation to avoid collisions)
      const memberString = sortedMembers.join(',');
      
      // Use a more robust hash function for better collision resistance
      let hash = 0;
      ...
      // Create a stable channel ID based on member hash (no timestamp for consistency)
      const hashString = Math.abs(hash).toString(36);
      const prefix = sortedMembers.length === 2 ? "dm" : "group";
      let channelId = `${prefix}-${hashString}`;
      
      // Ensure ID stays under Stream's 64 character limit
      if (channelId.length > 64) {
        // Fallback: use first 30 chars of hash to stay under limit
        const shortHash = hashString.substring(0, 30);
        channelId = `${prefix}-${shortHash}`;
      }
