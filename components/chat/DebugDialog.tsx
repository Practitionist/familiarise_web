...
export const DebugDialog = ({ userId, variant = "outline", className = "w-full" }: DebugDialogProps) => {
  ...
  const ActivityStats = () => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="p-3 border rounded-lg text-center">
        <div className="text-2xl font-bold text-green-600">{connectionStats.reconnectCount}</div>
        <div className="text-sm text-muted-foreground">Reconnects</div>
      </div>
      <div className="p-3 border rounded-lg text-center">
        <div className="text-2xl font-bold text-purple-600">{connectionStats.latency}ms</div>
        <div className="text-sm text-muted-foreground">Latency</div>
      </div>
      <div className="p-3 border rounded-lg text-center">
        <div className="text-2xl font-bold text-orange-600">
          {Math.floor((Date.now() - connectionStats.lastActivity.getTime()) / 1000)}s
        </div>
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-6xl max-h-[80vh] overflow-hidden">
        ...
        <Tabs defaultValue="overview" className="h-full">
          ...
          <TabsContent value="overview" className="space-y-4 h-[60vh] overflow-auto">
            {debugData && (
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="p-3 border rounded-lg">
                      ...
                      <div className="text-muted-foreground">{debugData.user.id}</div>
                    </div>
                    <div className="p-3 border rounded-lg">
                      <div className="font-medium">Role</div>
                      <div className="text-muted-foreground">{debugData.user.role}</div>
                    </div>
                    <div className="p-3 border rounded-lg">
                      <div className="font-medium">Name</div>
                      <div className="text-muted-foreground">{debugData.user.name || 'N/A'}</div>
                    </div>
                  </div>
                </div>
              </>
          ...
          <TabsContent value="connection" className="space-y-4 h-[60vh] overflow-auto">
            <ConnectionStatusCard />
            <ActivityStats />
            ...
            {streamConnection?.error && (
              <div className="p-4 border border-red-200 bg-red-50 rounded-lg">
                ...
                <div className="text-sm text-red-600 mt-1">{streamConnection.error}</div>
                <Button
                  variant="outline"
                  size="sm"
          ...
          <TabsContent value="activity" className="space-y-4 h-[60vh] overflow-auto">
            <ActivityStats />
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={connectionStats.latency > 1000 ? "text-red-500" : "text-green-500"}>
                      {connectionStats.latency}ms
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Connection Health:</span>
                    <span className={streamConnection?.chatConnected && streamConnection?.videoConnected ? "text-green-500" : "text-orange-500"}>
                      {streamConnection?.chatConnected && streamConnection?.videoConnected ? "Excellent" : 
                       streamConnection?.chatConnected || streamConnection?.videoConnected ? "Partial" : "Poor"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>{Math.floor((Date.now() - connectionStats.lastActivity.getTime()) / 1000)}s ago</span>
                  </div>
                  <div className="flex justify-between">
