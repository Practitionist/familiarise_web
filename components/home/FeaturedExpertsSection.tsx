...
function ExpertCard({ expert }: { expert: TConsultantProfile }) {
  return (
    <Link href={`/explore/experts/${expert.id}`} className="block flex-shrink-0 w-[300px] mx-3">
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-zinc-900 truncate">{expert.user.name}</h4>
              <p className="text-sm text-zinc-500 truncate">{expert.specialization || expert.domain.name}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-4">
            <div className="flex items-center gap-1">
              ...
              <span className="font-medium text-zinc-900">{expert.rating.toFixed(1)}</span>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {expert.tags?.slice(0, 3).map((tag) => (
              <Badge key={tag.id} variant="secondary" className="bg-zinc-100 text-zinc-700 hover:bg-zinc-200 text-xs border-0">
                {tag.name}
              </Badge>
            ))}
          </div>
...
export function FeaturedExpertsSection({ experts, isLoading }: FeaturedExpertsSectionProps) {
  return (
      ...
      <div className="container mx-auto px-4 md:px-6 mb-12 relative z-10">
        ...
        >
          ...
          <Link href="/explore/experts">
            <Button variant="outline" className="group border-zinc-300 hover:bg-zinc-100">
              View All Experts
