...
export const fetchConsulteeProfile = async (consulteeId: string) => {
  ...
  if (!response.ok) throw new Error(`Consultee profile fetch failed: ${response.statusText}`);
  const data = await response.json();
  return data.data;
};
...
export const fetchUserDetails = async (userId: string) => {
  ...
  if (!response.ok) throw new Error(`User details fetch failed: ${response.statusText}`);
  const data = await response.json();
  return data.data;
};
      ...
      if (process.env.NODE_ENV === 'development') {
        results.forEach((result, index) => {
          if (result.status === 'rejected') {
            console.warn(`Consultee prefetch failed for query:`, queries[index].queryKey, result.reason);
          }
        });
      }
    };
