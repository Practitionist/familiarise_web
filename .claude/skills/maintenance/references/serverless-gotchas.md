...
Phase-2 work inside `after()` (earnings, channel creation, notifications) can still be lost: the 60 s execution ceiling counts it, a crash ends it, and a bare `after()` THROWS outside a request scope (jest, `scripts/`, any lib path a script drives — `lib/api/after-safe.ts::scheduleAfter` degrades to a floating promise there). Bound it (5 s deadlines, SDK `timeoutMs`), never put money truth there, and keep an idempotent sweep (cron/ticker) that heals what `after()` missed.

Verified 2026-09-18: Netlify lists `next/after` as **full support** — it is backed by `context.waitUntil`, the invocation is held until the promise settles (up to the execution limit), the held time is billed as duration, and it applies to functions deployed on or after 2025-03-20. So `after()` is not silently dropped on this host; the rule above is about the ceiling and the scope, not about the platform forgetting the work. The durable shape for a notice is therefore: write the outbox row (`FailedEmail` / `NotificationOutbox`) inside the business transaction — reading recipients through `tx`, never the global client — and run only the vendor attempt inside `after()` (#1700, review round 2).
...
- A new instance's first ~25 s can stall the event loop. There are two ceilings, not one: the Lambda execution limit is 60 s (invocations of 32–39 s complete here), but the edge returns a 504 at ~26 s to any response that has not started streaming, so a lock retry budget or a silent admin route must still fit under ~25 s (#1454; `deployment/netlify/platform-limits.md`).
