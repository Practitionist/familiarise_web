---
description: Create Razorpay subscription checkout — hosted checkout, pending dedup, popup-blocked fallback, signature verification. Use when the user asks to "add subscription billing", "set up recurring payments", "create hosted checkout", "implement subscriptions", or needs a checkout flow for recurring charges.
...
2. **`customer_notify` is a boolean**: Pass `true`/`false`, not `1`/`0`.
...
8. **Offers**: Offer creation is Dashboard-only, but linking/unlinking an offer to a subscription is API-supported (`POST .../offers`, `DELETE .../offers/:offer_id`).
