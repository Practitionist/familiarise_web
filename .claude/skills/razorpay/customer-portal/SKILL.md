  ...
  try {
    ...
    return Response.json({
      ...
      subscription: {
        ...
        // current_end (Razorpay's field, not Stripe's current_period_end) is Unix seconds
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // NOTE: "cancelling" is an APP-INTERNAL status, not a Razorpay state.
    // On Razorpay the subscription stays `active` until the cycle ends, then becomes `cancelled`.
    await db
      .update(subscriptions)
      .set({
        cancelledAt: new Date(),
        status: immediate ? "cancelled" : "cancelling",  // "cancelling" = active until period end (internal)
        updatedAt: new Date(),
      })
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // pause_at: "now" pauses immediately; the only documented value is "now"
...
You don't have to cancel and recreate the subscription to change the payment method. Send the customer back to the existing subscription's `short_url` to re-authorize — the `short_url` persists and is returned by `razorpay.subscriptions.fetch(id)`.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // short_url persists — fetch it back and send the customer to re-authorize
    const rzpSub = await razorpay.subscriptions.fetch(
    ...
    return Response.json({
      ...
      message:
        "You'll be redirected to re-authorize payment with your new card on your existing subscription.",
    });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Create fresh subscription — Razorpay auto-creates the customer at the
    // authentication transaction, so no customer pre-creation or customer_id is needed.
...
3. **Razorpay auto-mints NON-GST invoices**: Each billing cycle auto-creates a Razorpay invoice entity (fetch via `razorpay.invoices.all({ subscription_id })`), each with a `short_url`. But these are NOT GST-compliant — a GST invoice is still your job (self-generate via `razorpay.invoices.create()` with GST line items, or issue from the Dashboard). Store the `razorpay_invoice_id` and `short_url` for any you self-generate.
...
8. **Payment method update is in-place**: Send the customer back to the subscription's `short_url` (which persists — `razorpay.subscriptions.fetch(id)` returns it) to re-authorize. No need to cancel and recreate.
