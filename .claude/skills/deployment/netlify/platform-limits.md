...
| Edge response for a non-streaming body | Confirmed by Netlify support on 2026-09-16: "the edge function timed out" at ~37–38 s from receipt if no response has begun streaming; not configurable on Pro; Enterprise can extend after account review | 504 "Inactivity Timeout" at ~26–27 s while the function keeps running and its DB write lands (#1454); a June 2026 forum thread reports a function finishing at 45 s after the client's 504                                                                                         |
...
| Background function                    | 15 minutes; immediate 202; retry after one minute, then after two more                                                             | First used by `reconcile-ledgers-background` (#1454). Not executed on deploy previews: a kick answers 202, leaves one empty log record and never runs the Lambda (verified 2026-09-13 with trivial functions in both declaration forms); prove one on a published or branch deploy |
...
## What Netlify said (ticket #1112198, answered 2026-09-16)

Netlify's engineer confirmed the fixed ~28 s window on new instances as "a runtime platform pattern, not a code pattern", attributed it to serialised initialisation when several Lambda environments are created at once ("at the account/region/function concurrency level"), and confirmed that requests bound for warm instances during a burst are held in the edge layer. The operative facts for planning: there is no fix in flight; **no plan, including Pro, offers provisioned concurrency or minimum instances**, and there is no public commitment; the edge returns "the edge function timed out" at roughly **37–38 s from receipt** if no response has started streaming, and that deadline is **not configurable on Pro** (Enterprise can extend it after account review); the classic `netlify.toml` bundling keys (`node_bundler`, `included_files`, `external_node_modules`) are **silently ignored** for the handler `@netlify/plugin-nextjs` v5 generates; Background Functions **do not execute on Deploy Previews or branch deploys** (a documentation gap they logged); "Init Duration" is not surfaced (feature request logged). Their mitigations, in their order: three to five *parallel* keep-warm pings, early response streaming, an Enterprise trial, or moving the function region (they named `us-east-1`, which would put every database round trip ~200 ms from Supabase Mumbai). Their own logs show the ~37–38 s 500 pattern across production deploys for the preceding 48+ hours. One caveat recorded here rather than accepted: AWS's own documentation puts a Lambda cold start at 100 ms–5 s and describes no 28-second serialisation, so "account-level" most plausibly means Netlify's shared regional account pool — the same thing from this site's seat, but a property of how Netlify runs Lambda rather than of Lambda.

### The second answer (2026-09-17)

Asked where the ~28 s serialisation lives, Netlify's engineer answered that it is **contention in Netlify's shared AWS account pool for `ap-southeast-1`** — other Netlify customers' functions in the region, not a property of this handler — and that the region is therefore "a genuine variable, not just a workaround": `us-east-1` has a significantly larger Lambda burst pool than `ap-southeast-1`, which he described as one of AWS's smaller regions for burst capacity. The trade-off is the ~200 ms per database round trip that #932 measured from the US against Supabase in Mumbai. He confirmed that **no tier, including Enterprise, offers a warm floor of N instances or provisioned concurrency**, as a plan or as an exception; Enterprise buys a longer edge receive-timeout and account support, which "buys execution time after the stall, not a shorter stall".

On keep-warm and billing: execution environments in `ap-southeast-1` are typically reclaimed after **roughly five minutes of complete inactivity**, so a ping every five minutes "races the reclaim window and you'll see cold starts bleed through" — three to four minutes is the reliable interval, and the zero-import probe is the right target. Keep-warm invocations are billed exactly like user-facing ones. **A cold-start stall is billed as duration**: Lambda charges wall-clock time from environment initialisation to response, so a 28 s frozen instance at 1,024 MB bills about 28 GB-seconds (≈0.093 GB-hours for a burst of twelve) although the application ran for ~200 ms of it. On this site's own logs over a twelve-hour sample on 2026-09-17, three cold invocations out of fifty-two accounted for 63% of the billed duration. Pro has no spend cap, only usage alerts at 50/75/90/100%. Invocation overage on the legacy `orb-pro` plan is a **step structure** — a fixed add-on charge raises the ceiling to 2,000,000, then per-500,000 increments — with the exact rates on Team settings → Billing or from the billing team in writing; GB-hour overage likewise. A plan that moves to credit-based billing at renewal meters the same usage as credits, draws overages from a balance, and **allows a credit budget cap**, which the legacy plan does not.

## What is still open with Netlify

The stall itself is no longer open: Netlify confirmed it on 2026-09-16 (see "What Netlify said" above) and there is nothing further to ask them that would change the application's options. Three follow-ups are outstanding on ticket #1112198: which limit produces the ~28 s serialisation (Netlify's shared regional account pool or a per-function property), whether any tier offers a warm floor of N instances rather than a longer timeout, and whether keep-warm invocations count against the plan's included 125,000. Two documentation gaps Netlify logged from the ticket — Background Functions not executing on Deploy Previews and branch deploys, and the edge receive-timeout beside the documented 60 s execution limit — and one feature request (surface `Init Duration` in the function log view) are theirs to close. The decision that follows from all of this is ADR 32, whose remaining input is the Vercel burst measurement in `docs/perf/vercel-experiment-runbook.md`.
