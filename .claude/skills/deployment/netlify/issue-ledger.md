...
Found on 2026-09-17 in the production function logs and fixed on 2026-09-18 (#1686): a scheduled function answering 5xx is re-invoked up to three times within about ten seconds, so the ticker's "answer 500 when a target failed" rule had it running at three times its cadence for as long as `reconcile-payment-status` kept failing on permanently unknown gateway references and `reconcile-orphaned-confirmations` kept re-driving pairs the DM gate refuses; the ticker now always answers 200 and both sweeps dead-letter those states.
