...
Issues: #866 (closed, superseded by #1390 and ADR 27), #1010 (open), #1517 (open, PR #1631), #1411 (closed), #1634 (open).

Found on 2026-09-13 while exercising the twins: `CRON_SECRET` existed on Netlify only in the `deploy-preview` context, so from the day the ticker shipped (2026-09-04) every production tick failed to authenticate and every HTTP twin answered 401 on production; GitHub Actions kept running the jobs directly, so nothing stopped, but the five-minute cadence never existed. The secret was set for `production` and `branch-deploy` and production redeployed the same day; the first live ticks then exposed three routes that fail every tick (`reconcile-refunds` 500, `reconcile-payment-status` 500, `reconcile-orphaned-confirmations` over the 6 s per-route budget with all channel ensures failing), tracked on #1634. A keys-only drift check against production cannot see a preview-only secret; the per-context presence recipe in `maintenance/references/netlify-env-sync.md` can.

**2026-09-18, #1708:** two of those three were the #1645 class again — a permanent per-row state retried and reported as a run failure every tick. `reconcile-payment-status` answered 500 because thirteen seeded payments carried gateway ids the gateway does not know (Stripe `resource_missing`, Razorpay 400), and `reconcile-orphaned-confirmations` overran its 6 s budget because appointments whose parties fail Stream's DM rule headed the `chatChannelEnsuredAt IS NULL` queue on every tick. The fix classifies an unknown gateway id as `unresolvable` (207, one Sentry warning per run, no `Payment` write per ADR 21), stamps a DM-blocked pair out of the queue as `channelsSkipped`, and gives the second target a 20 s budget with a bite of ten.
**2026-09-18, #1686:** the third was the ticker itself. A scheduled function answering 5xx is re-invoked up to three times within about ten seconds (observed, not documented), so the #1390 "answer 500 when a target failed" rule had the ticker running at three times its cadence for as long as #1708's two sweeps kept failing — every invocation estimate built on one tick per slot was three times low. The ticker now always answers 200 and reports through the `failed` list in its logged body.
