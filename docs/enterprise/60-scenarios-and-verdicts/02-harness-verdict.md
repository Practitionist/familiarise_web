...
| 35 | LICENSED_SEAT cap counts engagements across plan types | ✅ | One `Appointment` = one engagement (#710). CONSULTATION/WEBINAR debit 1 at checkout; COHORT debits N at enrolment; SUBSCRIPTION debits 1 per allocation lazily. Counter increment is a guarded conditional UPDATE (cap-check + increment can't race). |
