...
- **TDS** — `computeTdsForPayout` (`lib/compliance/tds.ts`). Default section **194-O** for ECO payouts (Familiarise is the e-commerce operator); `ConsultantProfile.tdsSection` overrides to 194J/194C; a Section 197 cert applies `tdsLowerRateCert` + rate; PAN fallback (206AA) withholds punitively. DTAA rates apply to non-residents only when strictly lower. **For the current rate, defer to [`../compliance/01-tds-overview.md`](../compliance/01-tds-overview.md)** — the compliance docs are authoritative on rates. `createOrgPayoutBatch` deducts TDS before dispatch; the `ORG_PAYOUT` posting's `Cr TDS_PAYABLE` records the withheld amount. Form 26Q/27Q export is backlog.
