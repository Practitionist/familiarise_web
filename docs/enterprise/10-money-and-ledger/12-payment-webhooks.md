...
| refund | `refund.created` | Resolve `payment_id` → `order_id`, then `handleRefundCreated` (status `created`/`pending` → `PENDING`). |
| refund | `refund.processed` | `handleRefundCreated` with status `processed` → `SUCCEEDED`, runs `applyRefundCascade`. |
