...
Both tables are immutable at the database, not merely by application convention: `prisma/sql/ledger-triggers.sql` installs `ledger_entry_immutable` and `ledger_transaction_immutable`, `BEFORE UPDATE OR DELETE` triggers that raise on any attempt to touch a posted row (owner decision Q5). Nothing in the codebase updates or deletes a journal row — the balance fold upserts `LedgerAccountBalance` only — so the trigger pair closes off the one write path that was previously only a code-review convention.
...
A payout already claimed against `ORG_PAYOUT` can still be clawed back after the fact — a dispute lost on one of its earnings, or a later refund cascade touching a booking it already paid out. `postPayoutClawback` (`lib/payments/operations/reversal-engine.ts`) posts `Dr CASH / Cr ORG_PAYABLE`, keyed `clawback:<refundId>:<payoutId>`, and every writer that can trigger a clawback — the dispute-lost path and both refund-cascade paths — calls the same function in the same transaction as the earnings counter it updates, so the reconciler's `LEDGER_DUAL_WRITE_GAP` finding, which compares the payout's stamped clawback total against the sum of the CASH debit legs actually posted, never sees a gap from a path that forgot to post (#1582 C-P1-02c).
...
A booking is refunded (fully or partially). The original booking legs are reversed **proportionally**: the fee/payable/GST credits become debits, and the funding legs become credits. `PLATFORM_FEE` carries the balancing _plug_ so the set ties out.
