...
The grouping is conceptual, not a code boundary (all fourteen run in one pass); it's how to _triage_: an ① finding means the journal itself is wrong (rare, high-severity — a manual SQL edit or a writer that bypassed `postLedgerTxn`); ②–④ mean a _cache_ drifted from a sound journal (fix the writer, re-derive the cache via a counter-transaction).
