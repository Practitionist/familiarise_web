...
The admin whole-event refund route reverses org-funded (`INTERNAL`-rail) child seats through the same `CLASS_MULTI` reversal the single-payment door uses, and now notifies the payer for exactly those children — a bell and a receipt email, sent best-effort after commit — because the single-payment door and `CREDITS`-rail seats already stage that notice inside `refundBookingPayment`, and `GATEWAY`-rail seats are told separately by the `refund.processed` webhook; the route deliberately skips notifying gateway seats itself to avoid a double notice (#1586). `createRazorpayRefund` now validates its amount as a positive safe integer before the gateway call instead of falling back to a full refund on a `0` or `NaN` input, and it throws `NOT_CAPTURED` up front when the underlying order has no captured payment rather than guessing at `items[0]` (#1584 P2-P0-01, P1-GW01b).

Inside the claimed transaction the cascade performs, in order: a **proportional PaymentLeg reversal** with the last leg absorbing the floor remainder, where a `WALLET` leg credits the org wallet back via `walletCredit`, an unbilled `INVOICE_ACCRUAL`/`OVERAGE_INVOICE_ACCRUAL` leg is netted through a negative `*_REVERSAL` sibling leg — the original leg is never mutated and the monthly rollup bills the net of the pair (#786; a PAID invoice instead defers to clawback), and `CARD`/`REFERRAL_CREDIT`/`LICENSE` legs are handled elsewhere; a **BookingUtilization reversal** that releases engagements proportionally; a **ConsultantEarnings** increment of `refundedShareAmount` capped at the consultant share; an **OrganizationEarnings** increment of `refundedAmountPaise` (org share only, never the consultant slice); an **OrganizationPayout clawback** that increments `clawbackAmountPaise`, stamps `clawbackInitiatedAt`, and posts the `Dr CASH / Cr ORG_PAYABLE` clawback journal entry (`clawback:<refundId>:<payoutId>`) in the same transaction when the earnings already rolled into a `COMPLETED` payout — the dispute-lost path and both refund-cascade paths call the same `postPayoutClawback` helper, so the reconciler's dual-write-gap check never sees a path that updated the counter without posting (#1582 C-P1-02c); the **credit-note mint** (§5); and finally a balanced **`REFUND` ledger transaction** (`idempotencyKey = refund:<refundId>`) where `PLATFORM_FEE` is the residual plug that absorbs the ≤3-paise floor remainder so the posting always balances. The ledger post no longer silently swallows a failure: because the cascade runs inside the caller's transaction, a posting failure records a durable `SystemEvent` on its own connection (so the row survives the rollback) and fires a P0 Sentry alert, then **re-throws** to roll the whole cascade back rather than half-applying the earnings/leg reversals with no balanced journal (#812). The caller — the refund cron or the gateway webhook — re-drives the refund, so the customer refund is not lost; it is re-applied atomically. The nightly reconciler's `EARNINGS_LEDGER_DRIFT`/`LEDGER_TXN_IMBALANCE` invariants remain the backstop that catches any residual divergence (see [ledger integrity](13-ledger-integrity.md)).

**Subscription tranches (#1766, PR-Z2).** A subscription's earnings are one row per cycle rather than one per payment, and the refund is the buyer's undelivered sessions, so the `ConsultantEarnings` step above is no longer a flat proportion of every row for those payments. The cascade computes one clawback, `prorate(Σ tranche share, refundPaise, payment.amount)`, and hands it to `allocateCycleClawback` (`lib/payments/payouts/earnings-reversal.ts`), which consumes the newest tranche first: rows whose `holdUntil` is still NULL (undelivered cycles), then rows whose hold is running or released, then rows already `BATCHED`, then rows already `PAID`, each absorbing at most `consultantSharePaise − refundedShareAmount`. The `CONSULTANT_PAYABLE` debits of the `REFUND` journal read what each row actually absorbed, so the ledger mirrors the rows. `refundEarnings` in `earnings-service.ts` uses the same allocator, `forceRefund` still gates a `PAID` row and the TDS reversal helper is unchanged. Rows without a `cycleOrdinal` — every non-subscription payment and every subscription accrued before PR-Z2 — keep the per-row proportion end to end. The quote that produces the refund amount in the first place, the unused-session rule in `quoteBookingRefund`, is described in the finance doctrine's refund section.

---

## 3. Gateway mechanics (Razorpay)

Razorpay's own refund object has its own lifecycle that sits _underneath_ our `Refund` row, and understanding it explains why our authoritative state always arrives by webhook rather than from the synchronous API response. A Razorpay refund progresses through `created` (the refund has been initiated), `pending` (Razorpay is attempting the transfer), `processed` (money returned to the customer — terminal success), and `failed` (terminal failure); see https://razorpay.com/docs/api/refunds/ and https://razorpay.com/docs/payments/refunds/faqs/. Our adapter maps `created`/`pending` to `PENDING`, `processed` to `SUCCEEDED`, and `failed` to `FAILED`.
...
Both minters now apply a **cumulative cap per invoice**, not a per-note cap: before writing a note they sum every `CreditNote.totalPaise` already issued against the same `OrganizationInvoice` inside the same transaction and credit at most the remainder, mirroring the consumer-side `mintConsumerCreditNote` cap. A partial refund followed by a later lost dispute on the same invoice are two independent idempotency keys, and neither one's own arithmetic knows about the other, so without the cumulative check the pair could reverse more than the invoice was ever billed for; once an invoice is fully credited, a further mint attempt is refused and recorded as `CREDIT_NOTE_FULLY_CREDITED` rather than either erroring or over-crediting (#1582 C-P0-01).

`applyRefundCascade` itself is clamped the same way before any of the reversals it fans out run: it computes the payment's refundable balance — `Payment.amount` minus every other returned `Refund` and every dispute already `LOST` or `CHARGE_REFUNDED` — inside the claiming transaction, and reverses at most that balance regardless of the amount the caller passed in. A cascade clamped this way records a `REFUND_CASCADE_CLAMPED` system event and Sentry warning carrying both the requested and the reversed figure, so the clamp is visible to ops even though it is not itself an error (owner decision Q3).
