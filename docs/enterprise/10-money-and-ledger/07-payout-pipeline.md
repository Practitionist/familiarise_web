...
The org rail carries the receivable, by design. An `ACTIVE` organisation on the INVOICE rail that has never paid an invoice does not park its members' bookings' earnings: `PENDING_TRUST` is applied only while the organisation is still `PENDING_VERIFICATION`, so consultants and host organisations are paid once their hold windows lapse, before the sponsoring organisation's NET-30 or NET-60 terms clear. The platform therefore extends the credit between payout and invoice settlement (ADR 12), and the invoice credit limit in `lib/enterprise/governance.ts` bounds that exposure per organisation, counting the booking being priced (#1744).
...
The consultant rail is unchanged by per-cycle subscription earnings (#1766, PR-Z2). A subscription now carries one `ConsultantEarnings` row per cycle, and `createPayoutBatch` still selects `READY` rows with a null `payoutId`, so three matured tranches of one payment ride into one payout, summed net of `refundedShareAmount`, while an undelivered tranche (still `PENDING` with a NULL `holdUntil`) stays out of the batch until its cycle completes. TDS is computed on the payout sum at disbursement, never per tranche, so the Section 194-O threshold arithmetic did not move.
