...
- **Placeholder `WalletEntry` top-up rows** past the grace window — reaped by `cleanup-abandoned-org-top-ups` cron. The placeholder itself is schema-level noise; the fact that a top-up was abandoned is captured in the audit log + the cron's output.
...
- **Design-partner customer set** (also informally "launch cohort") — the curated group of 2-3 enterprise customers onboarded during the first 3-6 months post-MVP to validate the enterprise tier. NOT related to the `Class` Prisma model (which is a B2C cohort-based course appointment type). The term appears in `27-design-partner-customer-set.md` where the selection criteria are spelled out.
