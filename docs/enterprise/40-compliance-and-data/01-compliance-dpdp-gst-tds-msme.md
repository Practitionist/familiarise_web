...
**Erasure and the payment vendors (#1771 rows 1 and 5).** Bank data is reference-only: custody of account numbers stays at RazorpayX, and the platform keeps only masked fields and the RazorpayX ids. After the scrub commits, `scrubUser` deletes every saved-card token on the buyer's Razorpay Customer and then clears `User.razorpayCustomerId`, and it deactivates each RazorpayX fund account and contact with `PATCH … { active: false }`. Inside the scrub transaction it nulls the PayoutAccount holder name, bank name, last four digits, IFSC and UPI id, while the RazorpayX ids, the `ConsultantPayout` rows and the TDS rows stay. A vendor failure never aborts the scrub: it is written as a `COMPLIANCE` system event and returned in `ScrubResult.vendorFailures`. The retained rows run on two clocks: Income-tax Rule 6F(5) keeps books of account for six years from the end of the relevant assessment year, and CGST Act section 36 keeps GST records for seventy-two months from the due date of that year's annual return. Erasure is still refused while money is in flight (`moneyInFlightForUser`).
