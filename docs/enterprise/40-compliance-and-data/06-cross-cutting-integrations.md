...
- **Why:** Export bundle is JSON-only (compliance reviewers prefer JSON; `jq -r '@csv'` for CSV). Erasure pseudonymizes the actor and retains the immutable money journal. See [compliance-dpdp-gst-tds-msme](01-compliance-dpdp-gst-tds-msme.md), [deletion-policy](02-deletion-policy.md), [data-export](03-data-export.md).
