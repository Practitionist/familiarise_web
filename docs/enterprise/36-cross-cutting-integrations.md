...
- **Schema:** `BillingAccount.walletBalance`, `WalletEntry`, `FundingLedgerEntry` (with `currency`).
- **Code paths:** [`lib/api/organizations/wallet.ts`](../../lib/api/organizations/wallet.ts) — `walletDebit()` and `walletCredit()`. Both write a `WalletEntry` + a paired `FundingLedgerEntry` inside the same `tx` so the ledger never goes out of sync.
- **Org dashboard surface:** `/billing`
- **Why:** Three-ledger discipline (Usage / Funding / Settlement) requires every wallet move to land in `FundingLedgerEntry`. The currency column there exists so a future multi-currency wallet (`#711`) doesn't need a backfill against historical writes.
...
- **Code paths:** [`jobs/billing/generate-subscription-invoices.ts`](../../jobs/billing/generate-subscription-invoices.ts) — `sendRenewalReminders()` at job entry then the existing cycle-advance + invoice + settlement-ledger transaction.
...
### B.7 Three-ledger discipline — ✅ Wired

- **Schema:** `UsageLedgerEntry`, `FundingLedgerEntry`, `SettlementLedgerEntry`, `LedgerReconciliationReport`.
- **Code paths:** Every money-moving site writes exactly one ledger row per side. Reconcile cron [`scripts/reconcile/reconcile-ledgers.ts`](../../scripts/reconcile/reconcile-ledgers.ts) asserts cross-ledger invariants nightly.
- **Why:** See `18-three-ledger-discipline.md`. The ledgers are write-once; any reversal is a new row, never an update of the original.
- **Future work:** none open.
...
- **Why:** Cap enforcement modes (BLOCK/CHARGE_MEMBER/CHARGE_ORG) and the engagement-counter pattern are described in `16-programs.md`. The booking transaction owns both the cap decision and the appointment commit.
...
- **Why:** Org-visible audit log must not leak engineering noise (stack traces, Prisma error syntax). The three-layer defense — write-side discipline, read-side scrub, channel separation — is documented in `35-system-events.md`.
- **Future work:** none open.

### G.2 SystemEvent (admin-only) — ✅ Wired

See `35-system-events.md`. Raw engineering payloads go here, never to `OrgAuditLog`.
