...
`23-runbooks.md` for the response procedures.
...
3. **Every alert has a runbook.** If you can't write the response
   procedure in `23-runbooks.md`, the alert is too vague — tighten it.
...
|-------|-----------|---------|
| Webhook queue backup | `count(webhook.razorpay.received) - count(webhook.razorpay.processed)` > 50 for 5 min | `23-runbooks.md#webhook-handler-is-backed-up` |
| Ledger reconciler failing | `reconcile.completed` with `ok=false` in the last 24h | `23-runbooks.md#ledger-reconciler-flagged-discrepancies` |
