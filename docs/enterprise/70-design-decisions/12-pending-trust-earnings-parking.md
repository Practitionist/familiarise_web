...
## Addendum (2026-09-20, #1766 PR-Z2) — subscription earnings are escrow per cycle

The hold this ADR describes assumed one earnings row per payment. A subscription is now the exception: `createEarningsFromPayment` mints one `ConsultantEarnings` row per cycle (`cycleOrdinal`), each starting in the same `PENDING` or `PENDING_TRUST` state this ADR governs but with `holdUntil` NULL, which means the cycle has not been delivered. The completion path stamps a tranche's hold when its cycle's last session completes, and a cancellation stamps whatever is still NULL so the value a refund leaves behind can still pay out. A refund claws back one proportion of the summed tranche share from the newest tranche first, so a delivered cycle is touched only after the undelivered ones are exhausted. The `PENDING_TRUST` park composes with this unchanged: a parked tranche can be stamped, and it still cannot reach `READY` until the trust cron promotes it.
