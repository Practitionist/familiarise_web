...
1. **Stay on Netlify and ask.** Send the support ticket drafted at `docs/perf/netlify-stall-ticket-draft.md`, reframed as a report of an undocumented pattern with isolation evidence, and wait. Cost: nothing. Outcome (2026-09-16/17): answered in full — no fix in flight, no warm floor or provisioned concurrency on any tier including Enterprise, the stall billed as duration, keep-warm reliable only at a three-to-four-minute interval. Staying on Pro in `sin` means living with the stall and paying for it.
...
3. **Stay on Netlify, move the functions to `us-east-1`.** Netlify's engineer stated on 2026-09-17 that the stall is contention in Netlify's shared regional pool and that `us-east-1` has a much larger burst pool, so this is a measurable option; the cost is the ~200 ms per database round trip #932 measured from the US against Supabase in Mumbai, paid on every query. Testable with a second Netlify site on the same repository.
4. **An always-on host** (Fly, Railway, Fargate; $6–33 per month). Removes cold starts outright and puts the process in Mumbai, at the cost of running our own deploy pipeline and preview environments.
...
The Netlify ticket went out on 2026-09-15 as #1112198 and was answered on 2026-09-16: Netlify's engineer confirmed the fixed window as "a runtime platform pattern, not a code pattern", confirmed the edge-layer holding of requests bound for warm instances, stated that no plan offers provisioned concurrency or minimum instances, that the ~37–38 s edge receive-timeout is not configurable on Pro, and that the only on-platform mitigation is several parallel keep-warm pings — which shipped as PR #1685 the next day as an interim. The Netlify half of the rule is therefore vendor-confirmed; the Vercel measurement remains the deciding input. The vendor's full answer is recorded in `.claude/skills/deployment/netlify/platform-limits.md` under "What Netlify said".
