...
Every cash event posts through `postLedgerTxn`, which is idempotent on a **structured, unique** key. A repeat is a no-op (`{ created: false }`); a racing duplicate hits the `@unique` `P2002`, aborts its tx, and the retry lands on the fast-path ([ledger & postings §2](../10-money-and-ledger/03-ledger-and-postings.md)).
