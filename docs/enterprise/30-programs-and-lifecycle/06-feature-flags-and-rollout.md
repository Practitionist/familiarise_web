...
| `ENABLE_CONSOLIDATED_INVOICE` | on       | Monthly invoice-accrual settlement cron (`settle-invoice-accruals`).                                                                                         | Opt-out, not opt-in: the job runs unless the variable is explicitly set to the string `"false"`. Absent runs; any other value runs. Documented here because it gates real money movement despite living outside `feature-flags.ts`.                                                                                                                                                                                    |
