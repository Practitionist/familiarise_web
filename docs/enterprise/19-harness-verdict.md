...
| 3 | Booking accrues a `PaymentLeg(source=INVOICE_ACCRUAL)` + `BookingUtilization` | ✅ | Checkout writes both rows in the same transaction. |
...
| # | Item | Verdict | Notes |
|---|------|---------|-------|
| 9 | `LICENSE` funding + LICENSED_SEAT Program with `coveredSessionsPerCycle=null` absorbs bookings | ✅ | `PaymentLeg(source=LICENSE, amountPaise=0)`; no wallet debit, no invoice line. |
