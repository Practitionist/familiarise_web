...
> **condition** and a **runbook anchor** (the _Alerts_ tables below). Open
...
_absence_ of progress, or on a backlog of un-stamped rows, not on a
...
_delivery_ leg that turns a cold row into something an on-call human
...
> _Alerts_ tables above are the contract for what each severity should
> route to.
