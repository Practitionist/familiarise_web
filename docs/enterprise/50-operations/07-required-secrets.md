...
| `RATE_CARD_SCOPED_RESOLUTION` | `lib/api/organizations/rate-card.ts` (`isScopedRateCardResolutionEnabled`), consumed by `resolveOrgSplit` in `lib/payments/payouts/earnings-service.ts` | Absent means off, which is the intended default (#1335). Off, settlement forwards only org scope, so a contract- or plan-scoped `RateCard` an org has created can never be selected and its bookings settle on the org default instead. Set it to exactly `on` to enable; any other value, including `true`, leaves it off. This one is not a credential — it is here because the scheduled fleet and the running application read the same money code, and a value set in only one of the two stores would settle the same booking two different ways depending on which surface accrued it. |
