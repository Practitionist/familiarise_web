...
| `SUPPLIER_STATE_CODE`     | `gst-outward-register-export`, the checkout tax engine                                                                                                                                   | The supplier's own state is unknown, so a B2C invoice cannot be placed under section 12(2)(b) of the IGST Act and the intra-state versus inter-state split cannot be decided.                                                             |
| `PLATFORM_INVOICE_PREFIX` | `gst-outward-register-export`                                                                                                                                                            | Optional. The consumer invoice and credit-note series falls back to the `FAM` prefix, which is a naming preference rather than a failure; set it only if finance wants a different series name.                                           |
