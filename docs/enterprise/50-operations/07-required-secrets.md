...
| `STRIPE_ALLOW_TEST_KEYS_IN_PRODUCTION`   | `initializeStripeClient` in `lib/payments/core/stripe.ts`, when the client is lazily constructed                                                                                                                                                                                         | Optional pre-launch opt-out, the Stripe twin of `RAZORPAY_ALLOW_TEST_KEYS_IN_PRODUCTION`. When it is exactly `true`, an `sk_test_` key under the production posture logs a loud error instead of throwing `STRIPE_TEST_KEY_IN_PRODUCTION`. Delete it and set a LIVE key before Stripe is ever enabled for customers.                                                                                                                                                                                                                                                                                                                                                  |
