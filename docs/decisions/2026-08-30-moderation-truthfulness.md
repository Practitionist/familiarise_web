...
The retry sweep ships with its HTTP entrypoint (`GET|POST /api/cleanup/retry-moderation-enforcement`, `CRON_SECRET`-gated) but without a scheduling workflow, because `.github/workflows` is owned by a sibling change. Until that workflow lands the sweep runs on demand only, and the staff Jobs catalogue lists it as "On demand" rather than implying a cadence nothing keeps. This is the one remaining gap between what the product claims and what it does, and it is a smaller one than the gap it replaces: the failure is visible on the report either way.
