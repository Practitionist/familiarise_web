...
A route that has just committed a business write stages every notice it owes — one `NotificationOutbox` row per bell and one `FailedEmail` row per email, so a recipient reached on both channels has two rows; no vendor call — before it returns, and runs the attempts inside `after()` through `scheduleAfter()` (`lib/api/after-safe.ts`). The rows are what make the notice durable: the relay finishes anything the attempt did not settle. Netlify holds the invocation for `after()` (`context.waitUntil`, full support), so the attempts normally run at once, but the stage must never depend on that — a crash, the 60-second ceiling or a scope without `after()` would otherwise lose the notice, and for `ORG_INVITATION` the email is the only channel that reaches someone without an account. The Novu helpers return their staged rows when given `{ tx: prisma }`; the onboarding senders expose a `stage*` twin of every `send*` and `attemptOnboardingEmail()` for the `after()` half.
