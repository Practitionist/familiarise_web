...
- **Non-blocking** -- notification calls are wrapped in try-catch and never block the calling operation; Novu failures are logged, while a failed or unconfigured Resend transactional send is persisted to `FailedEmail` and replayed by a retry worker rather than merely logged
