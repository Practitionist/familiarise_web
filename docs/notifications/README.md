...
- **A missing key is captured, not dropped** -- a missing Novu secret key for the current environment (`NOVU_DEVELOPMENT_KEY` or `NOVU_PRODUCTION_KEY`) makes a Novu trigger a no-op. On the Resend side `deliver()` always persists a failed send as a `PENDING` `FailedEmail` row, whether the cause is a missing `RESEND_API_KEY` (`EmailNotConfiguredError`, thrown inside its own try block) or a rejection Resend returned in `data.error`; a terminal cause (dead key, unverified domain, missing key, `validation_error`) is also paged in Sentry at level "error". The retry worker skips every row while the key is absent, replays it once the key exists, moves a terminal failure straight to `DEAD_LETTER` on that first attempt without rescheduling, and moves a transient failure to `RETRY` on the backoff ladder
