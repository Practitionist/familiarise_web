...
For **weekly** schedules: the validator compares the slot's day-of-week and time-of-day against the availability windows. It uses `dayOfWeekForStartsAt` enum, not `getUTCDay()` on the DateTime. If the stored DateTime has a reference date (e.g., 1970 epoch), `getUTCDay()` would return the wrong day.
...
| getUTCDay() on epoch dates     | Medium     | `validateMatchesSchedule` used `availStart.getUTCDay()` which fails for 1970 epoch reference dates. Now uses `dayOfWeekForStartsAt` enum (source of truth). Corrupt slots logged and skipped.                          | `SlotValidationService.ts`                                          |
