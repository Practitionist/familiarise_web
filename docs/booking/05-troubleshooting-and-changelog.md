...
| ClassPlan 30-min duration refine | Medium | `sessionDurationInHours` must be a multiple of 0.5 (matches webinar + atom math). Existing non-aligned plans fail validation on next PATCH. | `schemas/plans.ts` |
