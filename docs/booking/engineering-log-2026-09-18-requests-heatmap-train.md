...
- **#1732** — a lapsed **subscription** pay-link has no expiry path in either sweep: `cleanupAbandonedPayments` only matches appointments with a tentative occurrence, and a subscription's placeholder Appointment has none before allocation; `cleanupExpiredApprovalPendingPayments` reads consultations only. An unpaid subscription approval therefore stays `APPROVED_PENDING_PAYMENT` forever, counting against the consultant's open-request cap. Recorded during #1724 QA rather than widened into that PR.
