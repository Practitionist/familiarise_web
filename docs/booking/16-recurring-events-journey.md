...
A late joiner does not see recordings of the sessions that ran before their seat unless the host turns on "Late joiners can watch earlier recordings" (`ClassPlan.lateJoinersGetPastRecordings`). The class recordings list, the learner's recordings list and the single-recording read all apply that rule through `lib/stream/late-join-recordings.ts`.
