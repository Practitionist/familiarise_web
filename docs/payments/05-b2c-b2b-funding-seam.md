...
Schema source: `prisma/schema.prisma:3515–3563`.

---

## fundingSource Gates at Checkout

`BillingAccount.fundingSource` (`prisma/schema.prisma:901`, enum `FundingSource`) selects the money path:
...
Resolution happens in `lib/payments/operations/checkout.ts:1907`:
...
// isOrgSponsoredPayment = isOrgWalletPayment || isOrgInvoicedPayment || isOrgLicensedPayment
```

A synthetic `paymentIntent` id (prefix `org_wallet_` / `org_license_` / `org_invoice_`) is minted locally; no Razorpay/Stripe call is made. The `Payment` row is created with `paymentStatus: SUCCEEDED` immediately (`checkout.ts:2297`). Appointments are confirmed (non-tentative) in the same transaction — no webhook is required to complete the booking.

---

## PaymentLeg Sources and *_REVERSAL Siblings

Each `Payment` carries zero or more `PaymentLeg` rows (append-only; `prisma/schema.prisma:3568–3629`):
...
Ledger attribution: the double-entry reversal (Step 9, `refund.ts:764`) uses `payment.organizationId ?? walletOwnerOrgId` — `walletOwnerOrgId` is populated from `walletCredit.ownerOrgId` captured in Step 4, so the journal never loses its org anchor even on the invisible path.
...
checkout (WALLET-funded) → walletDebit → walletBalance -= booking amount  [checkout.ts:2322]

refund        → walletCredit → walletBalance += refund amount
              → WALLET_REFUND audit row
```

**WalletTopUp idempotency:** `providerOrderId` has a `@unique` constraint (`prisma/schema.prisma:1059`). A duplicate webhook delivery hits P2002 and is deduplicated — the wallet is never double-credited.
...
- Checkout funding resolution: `lib/payments/operations/checkout.ts:1809–1910`
