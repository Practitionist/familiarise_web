...
| -------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `STRIPE_ENABLED`                       | `assertGatewayUsable` in `lib/payments/validation/gateway-guards.ts`, at call time               | Any attempt to route a checkout to Stripe, or to mint a Stripe payment intent, throws a `DisabledGatewayError`. This is the fence that actually protects money. Set it to exactly `true` to open it.                                                       |
...
| `STRIPE_ALLOW_TEST_KEYS_IN_PRODUCTION` | `initializeStripeClient` in `lib/payments/core/stripe.ts`, when the client is lazily constructed | Under `NODE_ENV=production`, an `sk_test_` key throws `STRIPE_TEST_KEY_IN_PRODUCTION` instead of booting. Set it to `true` only for the pre-launch window where the production site legitimately runs on test keys, and delete it with the first live key. |
