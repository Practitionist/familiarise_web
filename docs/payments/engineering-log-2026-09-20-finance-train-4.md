...
The Home "needs you now" strip gains one row. `payoutSetupNeeded({ requirements, earningsCount })` in `lib/data/needs-you.ts` is true only when at least one earning exists and `requirements.currentlyDue` names `PAYOUT_ACCOUNT` or `ACCOUNT_VERIFICATION`; it is deliberately independent of `ENABLE_LIVE_PAYOUTS`, because the row exists to collect every account before the flag flips (a reason-based predicate would have stayed silent until launch, since `LIVE_PAYOUTS_OFF` outranks the account gates). `readPayoutSetupNeeded` reads the earnings count first and the requirements second, in sequence, and `getConsultantDashboard` carries the answer as `payoutSetup` with the flag so `deriveConsultantActionItems` can word the row — "Add your bank account — payouts begin at launch" while the flag is off, "Add your bank account to get paid" once it is on — linking to the settings route.
