...
## CI while a branch's schema is ahead of the live database

A pull request that renames tables cannot pass the sidecar and drift guards or `next build` against the live database, because the objects it declares do not exist there until the reset. Skipping those steps would leave exactly the riskiest pull request unverified, so `ci.yaml` does something else: when the pull request carries the `db-reset` label, one step rewrites `DATABASE_URL` and `DIRECT_URL` in the job's `.env` from the `REHEARSAL_DATABASE_URL` and `REHEARSAL_DIRECT_URL` secrets, which point at the rehearsal project, and the same guards and build run against the schema they belong to. The label also re-runs the job when it is added or removed. The recipe for a reset-riding branch is therefore: push the branch's schema to the rehearsal project (`db push`, `db:sidecars`, `db:assert-sidecars`, `db:seed`), set the two secrets, add the label. After the cutover, remove the label, delete the two secrets, and delete the rehearsal project.

## Rollback

The reset is reversible for as long as the platform holds only mock data, and the plan below assumes that. Decide to roll back if the post-reset smoke (`/`, `/api/health`, a booking, a join) fails and the cause is not obvious within fifteen minutes; do not debug on a broken production.

1. Before the reset, `pg_dump --format=custom` of the live database to a dated file outside every git tree (the runbook's `pg_dump` step). This is the point-in-time copy; the free Supabase plan keeps no backups of its own.
2. Code first: on Netlify, publish the previous production deploy and the previous `dev` deploy (Deploys → the last green deploy before the release → "Publish deploy"); both read the same database, so both go back together. On GitHub, revert the release merge on `prod` and the reset merge on `dev` (`git revert -m 1 <merge sha>`) so the next push does not redeploy the new code by accident.
3. Database second: with maintenance still OFFLINE, restore the dump (`pg_restore --clean --if-exists --no-owner` into the live project), then `npm run db:assert-sidecars` from the **reverted** checkout to confirm the old objects are back. If the dump is unusable, the equivalent is a reset backwards: `db push --force-reset` from the reverted checkout, then its `db:sidecars` and `db:seed` — the same runbook, run from the old schema.
4. Smoke the old deploys, lift OFFLINE, and re-run CI on the open pull requests. The reset PR stays merged-and-reverted until the cause is fixed on its branch; a second attempt repeats this runbook from the top, including the rehearsal.
