...
1. **B2C:** a dual take rate — 10% when the buyer arrives via the expert's own link, 20% when the marketplace sourced the buyer — decided 2026-09-18 (see ADR 33 and #1711; the code today still charges a flat 20% pending that work). The inverse-tiered idea (20%→12% by rate band) from month 10+ remains a future option worth revisiting once the dual rate has landed. Minimum prices enforced server-side.
