...
*Assumes 20% platform commission, 3% gateway fee (2.36% effective with GST)*
...
): { valid: boolean; minimumRequired?: number; maximumAllowed?: number; tier: ConsultantTier } {
  const config = TIER_CONFIG[tier];
  const minimumPrice = config.minPrices.CONSULTATION[durationMinutes] ||
...
export function suggestTierForPrice(
  ...
  for (const tier of tiers) {
    const config = TIER_CONFIG[tier];
    const minPrice = eventType === 'CONSULTATION'
