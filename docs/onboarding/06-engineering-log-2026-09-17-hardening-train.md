...
Each pull request ran a cold `tsc`, eslint on every touched file and its jest suites; #1700 additionally ran every payments and booking suite that imports the checkout and webhook modules it touches (69 suites), because bare `after()` throws outside a request scope. #1698 was exercised end to end on its Netlify deploy preview by a QA agent: the wizard through to a staff approval, the upload gate, the email-ownership refusal, the rate limiter tripping on the eleventh call, the role check, session revocation biting immediately, and the settings page — eight scenarios, all passing, with three pre-existing defects recorded and folded into PR-4 and PR-6. The review round on PR-5 (#1719) found one real bug and several ordering gaps, all fixed on the branch: the carry-over predicate `isValid: { not: false }` compiled to `<> false` and dropped every unreviewed document (a NULL row), so a re-file after a rejection carried nothing; the sweep deleted the storage object before claiming the row, which could strand a row a concurrent submission had just linked; the day-7 reminder stamped `reminderSentAt` outside the transaction that staged the email; the download route served a legacy row's declared type inline; the bell for `NEEDS_INFO` said "pending review"; and the legacy adapter still minted rows from client-supplied metadata, which is gone now that every upload has a row.
