...
`scripts/cleanup/sweep-verification.ts` runs daily at 03:20 UTC through `.github/workflows/sweep-verification.yml` and its HTTP twin `POST /api/cleanup/sweep-verification`. It deletes unlinked uploads older than 7 days (object first, then row), sends the day-7 reminder, and closes the day-14 stale requests, with the profile update and the decision bell and email staged in the same per-row transaction. It imports the storage leaf module (`lib/supabase-storage-core`) rather than `lib/supabase`, which carries `server-only` and cannot load in a bare Node process (#1270).
