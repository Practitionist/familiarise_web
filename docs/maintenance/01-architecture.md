...
When the cache is stale the cached-only reader returns the **last-known** state (not OFF), so an active window is still enforced while the refresh is in flight. A full document load calls `getMaintenanceState()`, which may return the cached state and reads Redis only after the 180-second edge cache expires, so any window is enforced within one document navigation once that cache window has passed (#1822).
