...
A non-retryable failure is reported to Sentry once at `warning` level for an account state and `error` otherwise, fingerprinted on `["stream-connect", kind, code]` and tagged `stream.failure` and `stream.code`, and no retry is scheduled. The video client's own catch logs at `warn` for the same reason; the outer catch owns reporting. On the server, `upsertUserToStream` logs the code 16 / 404 shape at `warn` and rethrows without a Sentry capture, since an account state is not an infrastructure failure.
