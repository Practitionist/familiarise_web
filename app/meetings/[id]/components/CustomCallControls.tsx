...
export const CustomCallControls: React.FC<CustomCallControlsProps> = ({
  ...
  const buttonBaseClasses = "flex flex-col items-center text-xs text-gray-300 hover:text-white";
  const iconWrapperClasses = "p-2 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors duration-150";
  const endCallButtonBaseClasses = "flex flex-col items-center text-xs text-red-400 hover:text-red-300";
  const endCallIconWrapperClasses = "p-2 rounded-full bg-red-800/50 hover:bg-red-700/60 transition-colors duration-150";
  ...
  return (
    <div className="flex justify-center items-center p-1.5 bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-lg space-x-2">
      ...
      {/* Separator */}
      <div className="h-6 w-px bg-gray-600 mx-2 self-center"></div> {/* Align separator centrally */}
      ...
      <div className="flex items-center space-x-2">
                 ...
                 {["Grid", "Speaker-Left", "Speaker-Right"].map((item) => (
                    <React.Fragment key={item}>
                      <DropdownMenuItem
                        ...
                        onClick={() =>
                          setLayout(item.toLowerCase().replace('-', '_') as CallLayoutType) // Ensure layout key format matches type if necessary
                        }
                      >
                        {item}
                      </DropdownMenuItem>
                      {/* Use theme color for separator */}
                      {item !== "Speaker-Right" && <DropdownMenuSeparator className="bg-border my-1 h-px" />}
                    </React.Fragment>
                  ))}
              </DropdownMenuContent>
            </DropdownMenu>
          ...
          <div className={buttonBaseClasses} title="Call Statistics">
            <button onClick={handleStatsClick} className="focus:outline-none border-none">
              <div className={iconWrapperClasses}>
          ...
          <div className={buttonBaseClasses} title="Toggle Participants">
            <button onClick={handleParticipantsClick} className="focus:outline-none border-none">
              <div className={iconWrapperClasses}>
          ...
          {!isPersonalRoom && (
             ...
             <div className={endCallButtonBaseClasses} title="End Call for Everyone">
