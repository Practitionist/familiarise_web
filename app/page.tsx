...
import { ProcessFlowDisplay, ProcessFlowStepProps } from "@/components/home/flows/ProcessFlowDisplay";
      ...
      <Card className="hover:shadow-lg transition-shadow duration-300 hover:-translate-y-0.5 h-full mx-3">
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">{expert.experience} experience</p>
          </div>
          <div className="flex flex-wrap gap-2 justify-center">
            {expert.tags?.slice(0, 3).map((tag) => (
              <Badge key={tag.id} variant="secondary" className="text-xs px-2 py-0.5">{tag.name}</Badge>
            ))}
          </div>
        </CardContent>
      </Card>
        ...
        <div className="flex items-start gap-3">
          <Avatar className="h-10 w-10 border border-gray-100">
            {review.consulteeProfile?.user?.image ? (
              <AvatarImage src={review.consulteeProfile.user.image} alt={review.consulteeProfile.user.name || "Reviewer"} />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start gap-2">
              <div className="min-w-0">
                <h4 className="font-semibold truncate">{review.consulteeProfile?.user?.name || "Anonymous"}</h4>
                <p className="text-sm text-gray-500 truncate">Review for {review.consultantProfile?.user?.name}</p>
              </div>
              <div className="flex items-center flex-shrink-0">
                {stars.map((star) => (
                  <Star key={star.id} className={`w-3 h-3 ${star.filled ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`} />
                ))}
              </div>
            </div>
...
const TestimonialLoadingSkeleton = () => {
  const skeletonIds = Array.from({ length: 3 }, (_, i) => `skeleton-${i}-${Math.random()}`);
   return (
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center">
            <div className="animate-pulse space-x-4 flex">
              {skeletonIds.map((id) => (
                <div key={id} className="w-[300px] h-[160px] bg-gray-200 rounded-lg flex-shrink-0" />
              ))}
            </div>
          </div>
        </div>
...
const BlurryBackground = () => {
  return (
    <div className="absolute inset-0 -z-10 overflow-hidden bg-transparent"> {/* */}
      ...
      <div className="absolute top-[20%] left-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-emerald-300 to-teal-400 opacity-25 blur-3xl animate-blob animation-delay-5000"></div> {/* MODIFIED: Was top-3/4 */}
      <div className="absolute top-[25%] right-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-rose-300 to-pink-400 opacity-25 blur-3xl animate-blob animation-delay-6000"></div> {/* MODIFIED: Was top-3/4 */}
      <div className="absolute top-[30%] left-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-emerald-300 to-teal-400 opacity-25 blur-3xl animate-blob animation-delay-5000"></div> {/* MODIFIED: Was top-3/4 */}
      <div className="absolute top-[35%] right-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-rose-300 to-pink-400 opacity-25 blur-3xl animate-blob animation-delay-6000"></div> {/* MODIFIED: Was top-3/4 */}
      <div className="absolute top-[40%] left-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-emerald-300 to-teal-400 opacity-25 blur-3xl animate-blob animation-delay-5000"></div> {/* MODIFIED: Was top-3/4 */}

      {/* Hero bottom corners - filling the empty spaces (ADJUSTED) */}
      <div className="absolute top-[38%] -left-32 -z-10 h-[600px] w-[600px] rounded-full bg-gradient-to-tr from-violet-200 to-purple-300 opacity-30 blur-3xl animate-blob animation-delay-1500"></div> {/* MODIFIED: Was top-1/2 -left-40 */}
      <div className="absolute top-[38%] -right-32 -z-10 h-[600px] w-[600px] rounded-full bg-gradient-to-tl from-sky-200 to-blue-300 opacity-30 blur-3xl animate-blob animation-delay-3500"></div> {/* MODIFIED: Was top-1/2 -right-40 */}
      
      {/* Hero bottom edge coverage (ADJUSTED) */}
      <div className="absolute top-[48%] left-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-emerald-200 to-teal-300 opacity-25 blur-3xl animate-blob animation-delay-5000"></div> {/* MODIFIED: Was top-3/4 */}
      <div className="absolute top-[48%] right-1/6 -z-10 h-[500px] w-[800px] rounded-full bg-gradient-to-t from-rose-200 to-pink-300 opacity-25 blur-3xl animate-blob animation-delay-6000"></div> {/* MODIFIED: Was top-3/4 */}

      {/* === TRANSFORM CAREER SECTION === */}
      {/* Upper-mid section coverage (ADJUSTED) */}
      <div className="absolute top-[55%] -left-12 -z-10 h-[300px] w-[300px] rounded-full bg-gradient-to-r from-green-200 to-cyan-300 opacity-35 blur-3xl animate-blob animation-delay-500"></div> {/* MODIFIED: Was top-1/3 -left-20 */}
      <div className="absolute top-[55%] -right-12 -z-10 h-[300px] w-[300px] rounded-full bg-gradient-to-l from-blue-200 to-green-300 opacity-35 blur-3xl animate-blob animation-delay-3200"></div> {/* MODIFIED: Was top-1/3 -right-20 */}
      <div className="absolute top-[60%] -left-24 -z-10 h-[300px] w-[300px] -translate-y-1/2 rounded-full bg-gradient-to-r from-cyan-200 to-sky-300 opacity-30 blur-3xl animate-blob animation-delay-4800"></div> {/* MODIFIED: Was top-1/3 -left-48 */}
      <div className="absolute top-[65%] left-1/3 -z-10 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gradient-to-r from-purple-200 to-pink-300 opacity-25 blur-3xl animate-blob animation-delay-7000"></div> {/* MODIFIED: Was top-1/4 */}
  ...
  return (
    <AnimatePresence>
      ...
      <main key="main-content-wrapper" className="flex-1 w-full relative">
        ...
        {/* HeroSection */}
        <section key="hero-section" className="relative py-12 md:py-24 lg:py-32 xl:py-40 overflow-hidden">
          ...
          <div className="container relative z-10 mx-auto px-4 md:px-6">
            <div className="flex flex-col items-center space-y-6 text-center mb-10">
              ...
              <p className="max-w-[700px] text-xl md:text-2xl text-gray-800">
                A platform where experts share their advice through 1-1 sessions, classes, webinars, and conferences.
              </p>
              <div className="flex space-x-4">
                <Link href="/explore/experts" className="inline-flex h-12 items-center justify-center rounded-md bg-gray-900 px-6 py-3 text-sm font-medium text-white shadow transition-colors hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-950">
        ...
        <section key="transform-career-section" className="w-full py-16 md:py-24 lg:py-32">
            ...
            <div className="grid items-center gap-12 lg:grid-cols-2">
              <div className="flex flex-col justify-center space-y-6">
                ...
                <ul className="space-y-4 text-lg text-gray-600 md:text-xl">
                  <li><span className="font-semibold">✓ Accelerate Your Growth:</span> Gain years of industry insights in just hours through our 1-1 sessions.</li>
                  <li><span className="font-semibold">✓ Expand Your Network:</span> Connect with industry leaders and peers in our exclusive classes and webinars.</li>
                  <li><span className="font-semibold">✓ Stay Ahead of the Curve:</span> Access cutting-edge knowledge and trends through our conferences.</li>
                </ul>
                <Link href="#" className="inline-flex w-full sm:w-auto items-center justify-center rounded-md bg-gray-900 px-6 py-3 text-sm font-medium text-white shadow transition-colors hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-950">
                  Start Your Journey
                </Link>
              </div>
              <div className="flex justify-center">
        ...
        <section key="unlock-potential-section" className="w-full py-16 md:py-24 lg:py-32">
          <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="grid lg:grid-cols-2 gap-12 mt-12">
              <div className="flex flex-col justify-center space-y-6">
                <ul className="space-y-6">
                  <li><h3 className="text-xl font-bold">Tailored Career Acceleration</h3><p className="text-gray-600">Receive a personalized roadmap to fast-track your career goals, designed by industry experts who've walked the path.</p></li>
                  <li><h3 className="text-xl font-bold">Insider Knowledge & Strategies</h3><p className="text-gray-600">Gain exclusive insights and proven strategies to navigate complex career challenges and seize hidden opportunities.</p></li>
                  <li><h3 className="text-xl font-bold">Confidence & Skill Mastery</h3><p className="text-gray-600">Develop unshakeable confidence and master critical skills through hands-on guidance and real-world application.</p></li>
                </ul>
              </div>
        ...
        {/* BestExpertsSection */}
        <section key="best-experts-section" className="w-full pt-6 md:pt-12 lg:pt-16 xl:pt-20">
          <div className="px-4 md:px-6 space-y-10 xl:space-y-16">
            <div className="grid max-w-[1300px] mx-auto gap-4 px-4 sm:px-6 md:px-10 md:grid-cols-2 md:gap-16">
              <div><h1 className="lg:leading-tighter text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl xl:text-[3.4rem] 2xl:text-[3.75rem]">The Best Experts in the World</h1></div>
              <div className="flex flex-col items-start space-y-4">
                <p className="mx-auto max-w-[700px] text-gray-500 md:text-xl dark:text-gray-400">Explore our wide range of consultants and find the right one for your business.</p>
        ...
        <section key="featured-experts-section" className="w-full py-12 md:py-24 lg:py-32">
          <div className="container mx-auto px-4 md:px-6 mb-12">
            <div className="text-center">
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter">Meet our Featured Experts</h2>
              <p className="mt-4 mx-auto max-w-[700px] text-gray-500 md:text-xl">We have a diverse team of experts ready to share their knowledge and expertise with you.</p>
              <Link href="/explore/experts"><Button className="mt-8 dark:bg-gray-800 text-white hover:bg-gray-700 transition-colors duration-300">View All Experts</Button></Link>
            </div>
          </div>
        ...
        <section key="offerings-section" className="py-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-12">Check out our various offerings</h2>
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {OFFERINGS.map((offering) => (
              <Card key={offering.title} className="rounded-lg shadow-md transition-transform duration-300 hover:-translate-y-2">
                ...
                <CardContent><p className="text-gray-600">{offering.description}</p></CardContent>
              </Card>
            ))}
          </div>
        </section>
                  ...
                  <div style={pageStyles['testimonials-marquee-track-ltr']}>
                    {marqueeGroups.map((group) => (
                      <div key={group.ltrId} className="flex">
                        {displayReviews.map((review) => <ReviewCard key={`${review.id}-${group.ltrId}`} review={review} />)}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div style={pageStyles['testimonials-marquee-track-rtl']}>
                    {marqueeGroups.map((group) => (
                      <div key={group.rtlId} className="flex">
                        {displayReviews.map((review) => <ReviewCard key={`${review.id}-${group.rtlId}`} review={review} />)}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section key="how-process-works-section" className="py-20">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h2 className="text-3xl font-bold mb-4">How The Process Works</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">Choose from our various learning formats and follow these simple steps to start your journey</p>
          </motion.div>
                  ...
                  {Object.entries(flowData).map(([flowType, steps]) => (
                    <TabsContent key={flowType} value={flowType}>
                      <div className="space-y-6">
                        {steps.map((step) => <ProcessFlowDisplay key={step.number} {...step} />)}
                      </div>
                    </TabsContent>
                  ))}
                </Suspense>
        ...
        <section key="join-community-section" className="w-full py-16 md:py-24 lg:py-32">
          <div className="container mx-auto px-4 md:px-6 text-center">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tighter mb-4">Join our Community of Experts</h2>
            <p className="text-lg text-gray-600 md:text-xl max-w-[600px] mx-auto mb-8">Share your expertise with people who need it and grow your personal brand.</p>
            <Button className="w-full sm:w-auto bg-gray-900 text-white hover:bg-gray-800">Become an Expert</Button>
          </div>
        </section>
              ...
              <Accordion className="w-full mt-4" type="multiple" defaultValue={[]}>
                {faqItems.map((item, index) => (
                  <AccordionItem key={`item-${index + 1}`} value={`item-${index + 1}`}>
                    <AccordionTrigger className="w-full text-left"><span className="flex-1">{item.question}</span></AccordionTrigger>
                    <AccordionContent><p className="text-sm text-gray-600">{item.answer}</p></AccordionContent>
                  </AccordionItem>
                ))}
              </Accordion>
            </CardContent>
          ...
          <div className="px-4 md:px-6">
            <div className="grid gap-6 items-center">
              <div className="flex flex-col justify-center space-y-4 text-center">
                <div className="space-y-2">
                  <h1 className="text-3xl font-bold tracking-tighter sm:text-5xl xl:text-6xl/none bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-500">Revolutionize Your Email Experience</h1>
                  <p className="max-w-[600px] text-zinc-200 md:text-xl dark:text-zinc-100 mx-auto">Join us and take control of your inbox. Fast, secure, and designed for modern life.</p>
                </div>
                <div className="w-full max-w-sm space-y-2 mx-auto">
                  <form className="flex space-x-2">
                    <Input className="max-w-lg flex-1 bg-gray-800 text-white border-gray-900" placeholder="Enter your email" type="email" />
                    <Button className="bg-white text-black" type="submit">Join Now</Button>
                  </form>
                  <p className="text-xs text-zinc-200 dark:text-zinc-100">Get ready to redefine your email experience. <Link className="underline underline-offset-2 text-white" href="#">Terms & Conditions</Link></p>
                </div>
              </div>
            </div>
          </div>
