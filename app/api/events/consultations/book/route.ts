...
export async function POST(request: Request) {
  try {
    ...
    const { consultationPlanId, preferredDateTime, requestNotes } = await request.json();
    ...
    if (!consultationPlan) {
      return NextResponse.json({ error: 'Consultation plan not found' }, { status: 404 });
    }
    ...
    if (existingRequests.length > 0) {
      return NextResponse.json({ error: 'Slot is currently pending approval for another request' }, { status: 409 });
    }
    ...
    if (consultationPlan.consultantProfile.scheduleType === 'WEEKLY') {
      ...
      const slotOfAvailability = await prisma.slotOfAvailabilityWeekly.findFirst({
        where: {
