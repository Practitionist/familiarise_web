...
async function fetchDomainsAndSubdomains() {
  ...
  return { 
    ...
    subdomains: domains.flatMap(d => d.subDomains.map(sd => ({ id: sd.id, name: sd.name, domainId: sd.domainId })))
  };
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ domains, subdomains }, tags, consultantMetadata] = await Promise.all([
    ...
    return NextResponse.json({ error: 'An error occurred while fetching metadata' }, { status: 500 });
  }
}
