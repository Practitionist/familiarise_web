...
import { TAppointment, TConsultation, TSubscription } from "@/types/appointment";
...
export async function GET(
  ...
  try {
    ...
    const [appointmentsRes, consultationsRes, subscriptionsRes] = await Promise.all([
      fetch(
    ...
    if (!consultationsRes.ok) {
      throw new Error(`Failed to fetch consultations: ${consultationsRes.statusText}`);
    }
    if (!subscriptionsRes.ok) {
      throw new Error(`Failed to fetch subscriptions: ${subscriptionsRes.statusText}`);
    }

    // Parse responses
    const appointmentsData: ApiResponse<TAppointment[]> = await appointmentsRes.json();
    const consultationsData: ApiResponse<TConsultation[]> = await consultationsRes.json();
    const subscriptionsData: ApiResponse<TSubscription[]> = await subscriptionsRes.json();

    // Transform appointments (same logic as fetchHelpers.ts)
    const transformedAppointments = appointmentsData.data.map((appointment) => ({
      id: appointment.id,
      ...
      consultation: appointment.consultation
        ? {
            ...
            consultationPlan: {
              ...
              consultantProfile: appointment.consultation.consultationPlan.consultantProfile as any,
            },
            ...
            requestedBy: {
              ...
              user: {
                ...
                image: appointment.consultation.requestedBy?.user?.image ?? null,
              },
            },
          }
        : undefined,
      subscription: appointment.subscription
        ? {
            ...
            subscriptionPlan: {
              ...
              consultantProfile: appointment.subscription.subscriptionPlan.consultantProfile as any,
            },
            ...
            requestedBy: {
              ...
              user: {
                ...
                image: appointment.subscription.requestedBy?.user?.image ?? null,
              },
            },
      ...
      webinar: appointment.webinar
        ? {
            ...
            webinarPlan: {
              ...
              consultantProfile: appointment.webinar.webinarPlan.consultantProfile as any,
            },
      ...
      class: appointment.class
        ? {
            ...
            classPlan: {
              ...
              consultantProfile: appointment.class.classPlan.consultantProfile as any,
            },
    ...
    const consultationApprovals = consultationsData.data.map((consultation: TConsultation) => ({
      id: consultation.id,
      type: "Consultation",
    ...
    const subscriptionApprovals = subscriptionsData.data.map((subscription: TSubscription) => ({
      id: subscription.id,
      type: "Subscription", 
