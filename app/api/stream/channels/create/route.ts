...
export async function POST(req: NextRequest) {
  try {
    ...
    const { channelType, eventId, eventType, channelName, members, createdById } = body;
    ...
    if (eventType && eventId) {
      ...
      console.log(`Creating ${eventType} channel for event ${eventId} by user ${createdById}`);
      ...
      try {
        ...
        console.log(`Successfully created ${eventType} channel for event ${eventId}:`, result);
      } catch (eventError) {
        console.error(`Error creating ${eventType} channel for event ${eventId}:`, eventError);
      ...
      if (!channelName) {
        return NextResponse.json(
          { success: false, error: "channelName is required for custom channels" },
          { status: 400 }
        );
      }

      const channelId = crypto.randomUUID();
      console.log(`Creating custom ${channelType} channel: ${channelId} with name: ${channelName}`);
