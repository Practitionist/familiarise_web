...
describe("Preferred Schedule Tests", () => {
  ...
  describe("Upcoming Slots", () => {
    it("should get all future preferred slots in order", () => {
      ...
      for (let i = 0; i < slots.length - 1; i++) {
        expect(slots[i].slotTime.getTime()).toBeLessThan(
          slots[i + 1].slotTime.getTime(),
        );
      }
  ...
  describe("Monthly Events", () => {
    it("should group events by month", () => {
      ...
      const januaryEvents = getPreferredMonthlyEvents(
        allEvents,
        new Date("2025-01-01"),
      );
      ...
      const decemberEvents = getPreferredMonthlyEvents(
        allEvents,
        new Date("2024-12-01"),
      );
