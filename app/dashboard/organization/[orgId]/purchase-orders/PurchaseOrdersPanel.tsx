...
/**
 * Purchase orders — a Billing tab since #1527 Q7 (India AP 3-way match).
 *
 * Thin orchestrator: state for filters + dialog open/close, plus the list
 * query. UI primitives live in `./components/*`, helpers in `./utils/*`. The
 * Billing page shows this tab only for `purchaseOrders.read` + canSponsor +
 * requiresPO; the server stays authoritative.
 */
...
export function PurchaseOrdersPanel({ orgId }: Readonly<{ orgId: string }>) {
    ...
    <>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <p className="max-w-2xl text-sm text-muted-foreground">
          Purchase orders back contracts and invoices. Cancel one to retire it
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              <CardDescription>
                Caller-issued PO numbers; uniqueness enforced per org. POs with
                attached contracts or invoices can only be cancelled, not
                deleted.
              </CardDescription>
            </div>
