...
import { OrgScopedEarnings } from "./OrgScopedEarnings";
...
const PAYOUT_RECIPIENT_DESCRIPTION: Record<string, string> = {
  SELF: "Your share of every session hosted via this org flows directly to your personal payout account. Manage that on your personal dashboard.",
  ORGANIZATION:
...
export default async function MyArrangementPage({
    ...
    return (
      <>
        ...
        <DashboardContent>
          <EmptyState
            variant="page"
            title="No hosted sessions here"
            description={`${access.org.name} sponsors sessions but doesn't host its own, so there is no compensation to show. Sessions you deliver are in Appointments.`}
            action={
              <Link
  ...
  return (
    <>
      <DashboardHeader
        title="Compensation"
        description={`How ${access.org.name} compensates you for sessions hosted under their brand.`}
      />
      <DashboardContent>
        {member.consultantProfileId && (
          <Section title={`Your earnings under ${access.org.name}`}>
            <OrgScopedEarnings orgId={orgId} orgName={access.org.name} />
          </Section>
        )}
                ...
                <tbody>
                  {upcomingSessions.map((s) => (
                    <tr key={s.id} className="border-t">
                      ...
                      <td className="px-4 py-2">{humanizeEnum(s.type)}</td>
                      ...
                      <td className="px-4 py-2 text-right text-xs">
                        ...
                        {s.earning
                          ? formatCurrencyAmount(
                              s.earning.consultantSharePaise,
                              "INR",
                            )
                          : "—"}
                      </td>
          ...
          {member.payoutRecipient === "ORGANIZATION" && payoutAccount && (
            <div className="mt-4 rounded-md border-l-2 border-primary bg-muted/40 p-3 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">
                ...
                {payoutAccount.accountNumberLast4} · status:{" "}
                {humanizeEnum(payoutAccount.status)}
              </p>
            </div>
          )}
  ...
  return (
    <div
      className={
        ...
        (emphasised ? "border-primary bg-primary/5" : "border-border bg-card")
      }
    >
