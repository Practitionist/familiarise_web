...
export function UnifiedCalendar({
  ...
  const renderTimeCell = (
    ...
    // lightweight placeholder eliminates the confusing, inactive "partially
    // booked" artefacts while preserving grid structure.
    if (status.isDisabled && !status.isAvailable) {
      return <div className="h-8 w-full bg-slate-100/60 rounded-sm" />;
    }

    const slotUTCTimestamp = status.intervalStartUTCString; // Stable key/id for this 30-minute cell
