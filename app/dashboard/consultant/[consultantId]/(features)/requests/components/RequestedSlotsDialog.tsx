...
export function RequestedSlotsDialog({
  ...
  const validateSlots = async () => {
    try {
      ...
      const endpoint = requestType === AppointmentsType.SUBSCRIPTION
          ...
          <>
            {hasConflicts && (
              <div className="bg-red-50 p-4 rounded-md mb-4">
                <h3 className="font-semibold text-red-700 mb-2">Conflicting Slots</h3>
                <ul className="space-y-2">
                  {conflicts.map((conflict, index) => (
                    <li key={index} className="text-sm text-red-600">
                      {new Date(conflict.slot).toLocaleString()} - Conflicts with {conflict.existingAppointment.type} 
            ...
            {hasOutsideSlots && (
              <div className="bg-yellow-50 p-4 rounded-md mb-4">
                <h3 className="font-semibold text-yellow-800 mb-2">Slots Outside Availability</h3>
                <ul className="space-y-2">
                ...
                <p className="mt-2 text-sm text-yellow-700">
                  These slots are outside your regular availability. You can override and allocate them anyway.
                </p>
              </div>
            )}
            ...
            {!hasConflicts && !hasOutsideSlots && (
              <div className="bg-green-50 p-4 rounded-md mb-4">
                <h3 className="font-semibold text-green-700">All Slots Available</h3>
        ...
        <DialogFooter className="gap-2">
          ...
          {!hasConflicts && !loading && validationResult && (
            <Button
              ...
              disabled={loading}
            >
              {hasOutsideSlots ? "Override and Allocate" : "Allocate Requested Times"}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
