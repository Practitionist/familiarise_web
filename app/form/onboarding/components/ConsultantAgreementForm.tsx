...
interface Props {
  onNext: (data: Partial<{ termsAccepted: boolean; privacyAccepted: boolean }>) => void;
  onBack: () => void;
  initialData: Partial<{ termsAccepted: boolean; privacyAccepted: boolean }>;
}

const ConsultantAgreementForm: React.FC<Props> = ({ onNext, onBack, initialData }) => {
  const [termsChecked, setTermsChecked] = useState(initialData.termsAccepted || false);
  const [privacyChecked, setPrivacyChecked] = useState(initialData.privacyAccepted || false);
