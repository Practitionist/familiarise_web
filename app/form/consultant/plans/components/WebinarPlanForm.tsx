...
const WebinarPlanFormSchema = z.object({
  plans: WebinarPlanSchema.array().min(
    1,
    "At least one webinar plan is required",
  ),
});

type FormData = z.infer<typeof WebinarPlanFormSchema>;

const WebinarPlanForm: React.FC<Props> = ({ onNext, onBack, initialData }) => {
  const {
    register,
    control,
    handleSubmit,
    ...
    >
      ...
      {fields.map((field, index) => (
        <div key={field.id} className="space-y-2 p-4 border rounded">
          ...
          <div className="space-y-2">
            <Label htmlFor={`plans.${index}.maxParticipants`}>
              Max Participants
            </Label>
            <Input
              id={`plans.${index}.maxParticipants`}
              type="number"
              {...register(`plans.${index}.maxParticipants` as const, {
                valueAsNumber: true,
              })}
            />
            {errors.plans && errors.plans[index]?.maxParticipants?.message && (
              <p className="text-red-500">
                {errors.plans[index]?.maxParticipants?.message}
              </p>
            )}
          </div>
