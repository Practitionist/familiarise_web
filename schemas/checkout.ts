...
export const consultationSearchParamsSchema = searchParamsSchema.extend({
  ...
  (data) =>
    ...
    (!data.slotOfAvailabilityWeeklyId && data.slotOfAvailabilityCustomId),
  {
    message: "Exactly one of slotOfAvailabilityWeeklyId or slotOfAvailabilityCustomId must be provided",
...
export const checkoutSchema = z.object({
  ...
  (data) => {
    ...
    if (["CONSULTATION", "SUBSCRIPTION"].includes(data.appointmentType)) {
      return data.slotOfAvailabilityWeeklyId || data.slotOfAvailabilityCustomId;
    }
    return true;
  },
  {
    message: "Consultation and subscription appointments require slot availability ID",
    path: ["slotOfAvailabilityWeeklyId"],
  }
).refine(
  (data) => {
  ...
  (data) => {
    ...
    if (data.slotStartTimeInUTC && data.slotEndTimeInUTC) {
      return new Date(data.slotStartTimeInUTC) < new Date(data.slotEndTimeInUTC);
    }
    return true;
  },
  {
...
export type ConsultationSearchParams = z.infer<typeof consultationSearchParamsSchema>;
export type SubscriptionSearchParams = z.infer<typeof subscriptionSearchParamsSchema>;
...
export type CheckoutSuccessResponse = z.infer<typeof checkoutSuccessResponseSchema>;
...
export const validateSearchParamsForAppointmentType = (
  ...
  switch (appointmentType) {
    ...
    default:
      return { success: false, error: { issues: [{ message: "Invalid appointment type" }] } };
  }
};
