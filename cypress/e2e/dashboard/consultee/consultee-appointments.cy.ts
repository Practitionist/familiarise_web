...
describe('Consultee Appointments Tests', () => {
    ...
    cy.request<{ consulteeIds: string[] }>("GET", "/api/user/consultees?idsOnly=true")
      .then((response) => { // Generate tests inside .then()
        ...
        if (!consulteeIds || consulteeIds.length === 0) {
          throw new Error("Failed to fetch consultee IDs or no consultees found.");
        }
                                ...
                                cy.get(`[data-testid="${type}-${event.id}"]`).within(
                                  () => {
                                    ...
                                    eventSlots.forEach((slot) => {
                                      const startTime = formatTimeForOverview(
                                        new Date(slot.slotStartTimeInUTC),
                                      );
                                      const endTime = formatTimeForOverview(
                        ...
                        .then((currentMonthText) => {
                          ...
                          const verifyMonth = () => {
                            ...
                            monthSlots.forEach((slot) => {
                              ...
                              const title =
                                slot.event[`${type.toLowerCase()}Plan`]?.title;
                              ...
                              cy.get(".grid.grid-cols-7")
                                ...
                                .within(() => {
                                  ...
                                  cy.log(
                                    `Looking for time: ${time} and title: ${title}`,
                                  );
                                  ...
                                  cy.get("button.w-full.text-left.text-xs")
                                    ...
                                    .and("contain", time);
                                  if (title) {
                                    cy.get("button.w-full.text-left.text-xs").should(
                                      "contain",
                                      title,
                                    );
                                  }
  ...
  it('successfully loads tests', () => {
    cy.log('Consultee appointment tests generated and loaded.');
  });
});
