...
describe("Consultee Dashboard", () => {
  ...
  beforeEach(() => {
    ...
    cy.get('[data-testid="slot-list"]', { timeout: 30000 })
      .should("exist")
      .then(() => {
        cy.readFile("cypress/logs/info.json").then((logs) => {
          logs.push({
          ...
          ([consultationsResp, subscriptionsResp, classesResp]) => {
            ...
            function getEventSlots(event: any) {
              ...
              if (event.tentativeSchedule) {
                try {
                  return JSON.parse(event.tentativeSchedule).map(
                    (slot: any) => ({
            ...
            const futureSlots = allSlots.filter(
              (slot: any) => new Date(slot.startTime) > now,
            );

            // Verify all future slots are displayed
            cy.get('[data-testid="slot-datetime"]').should(
              "have.length",
              futureSlots.length,
            );
            ...
            allEvents.forEach((event: any) => {
              ...
              eventSlots.forEach((slot: any) => {
                ...
                cy.get("body").then(($body) => {
                  ...
                  const found =
                    upcomingSlots.some((el) =>
                      el.textContent?.includes(expectedText),
                    ) ||
                    monthlySlots.some((el) =>
                      el.textContent?.includes(expectedText),
                    );
                  ...
                  expect(
                    found,
                    `Expected to find slot "${expectedText}" but it was not found in either section`,
                  ).to.equal(true);
                });
              });
            });
