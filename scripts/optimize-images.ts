...
async function processImage(imagePath: string): Promise<ImageResult | null> {
  ...
  try {
    ...
    const dimensions = execSync(`identify -format "%wx%h" "${imagePath}"`)
      .toString()
      .trim();
    ...
    if (width <= MAX_WIDTH) {
      console.log(
        colors.gray(
          `  - Image already within size limits (${width}x${height})`,
        ),
      );
    } else {
      console.log(
        colors.yellow(
          `  - Resizing from ${width}x${height} to fit within ${MAX_WIDTH}px width`,
        ),
      );
    }
    ...
    execSync(
      `cwebp -q ${QUALITY} -resize ${Math.min(width, MAX_WIDTH)} 0 "${imagePath}" -o "${webpPath}"`,
    );
    ...
    execSync(
      `avifenc --min 0 --max 63 --speed 6 --jobs 4 -a end-usage=q -a cq-level=25 -a tune=ssim "${imagePath}" "${avifPath}"`,
    );
    ...
    console.log(
      colors.green(
        `  - WebP: ${formatSize(webpSize)} (${webpSavings}% smaller)`,
      ),
    );
    console.log(
      colors.green(
        `  - AVIF: ${formatSize(avifSize)} (${avifSavings}% smaller)`,
      ),
    );
    ...
    console.error(
      colors.red(`  - Error processing ${relativePath}: ${errorMessage}`),
    );
    return null;
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(
    '2. Update your code to use the optimized images with the "next/image" component',
  );
