...
async function main() {
    ...
    try {
        ...
        const consultantChannels = await serverClient.queryChannels({ members: { $in: [consultant.id] } }, { last_message_at: -1 }, { limit: 30 });
        ...
        const consulteeChannels = await serverClient.queryChannels({ members: { $in: [consultee.id] } }, { last_message_at: -1 }, { limit: 30 });
