 ...
 */
async function isFolderEmpty(bucketName: string, folderPath: string): Promise<boolean> {
  try {
 ...
 */
async function getFoldersInPath(bucketName: string, path: string): Promise<FolderItem[]> {
  try {
    ...
    const folders = items.filter((item: FileObject) => 
      !item.name.includes('.') || item.metadata?.isFolder
    );

    return folders.map((folder: FileObject): FolderItem => ({
      name: folder.name,
 ...
 */
async function deleteEmptyFolder(bucketName: string, folderPath: string): Promise<boolean> {
  try {
    ...
    const placeholderFiles = items.filter((item: FileObject) => 
      item.name === '.keep' || 
      item.name === '.gitkeep' || 
    ...
    if (placeholderFiles.length > 0) {
      const filesToDelete = placeholderFiles.map((file: FileObject) => `${folderPath}/${file.name}`);
      ...
      if (deleteError) {
        console.error(`Error deleting placeholder files from ${folderPath}:`, deleteError);
        return false;
      }

      console.log(`✓ Cleaned up ${filesToDelete.length} placeholder files from ${folderPath}`);
    }
...
async function cleanupEmptyFolders(): Promise<void> {
  ...
  try {
    ...
    const appointmentFolders = await getFoldersInPath(bucketName, 'appointments');
    ...
    for (const appointmentFolder of appointmentFolders) {
      ...
      const consulteeFolders = await getFoldersInPath(bucketName, appointmentFolder.fullPath);
      
      if (consulteeFolders.length === 0) {
        console.log(`  📂 Appointment folder ${appointmentFolder.name} is empty`);
        
        const isEmpty = await isFolderEmpty(bucketName, appointmentFolder.fullPath);
        if (isEmpty) {
          const deleted = await deleteEmptyFolder(bucketName, appointmentFolder.fullPath);
          if (deleted) {
            foldersDeleted++;
            console.log(`  ✅ Cleaned up empty appointment folder: ${appointmentFolder.name}`);
          }
        }
        continue;
      }
      ...
      for (const consulteeFolder of consulteeFolders) {
        foldersChecked++;
        const isEmpty = await isFolderEmpty(bucketName, consulteeFolder.fullPath);
        
        if (isEmpty) {
          emptyConsulteeFolders++;
          const deleted = await deleteEmptyFolder(bucketName, consulteeFolder.fullPath);
          if (deleted) {
            foldersDeleted++;
            console.log(`    ✅ Cleaned up empty consultee folder: ${consulteeFolder.name}`);
          }
        } else {
          console.log(`    📄 Consultee folder ${consulteeFolder.name} contains files`);
        }
      }
      ...
      if (emptyConsulteeFolders === consulteeFolders.length) {
        const appointmentIsEmpty = await isFolderEmpty(bucketName, appointmentFolder.fullPath);
