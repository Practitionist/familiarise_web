  ...
  switch (true) {
    ...
    case /slots/.test(parentSegment):
      if (lastSegment === 'availability') {
        return [
          { key: 'startDate', value: '{{start_date}}', description: 'Start date (ISO)' },
          { key: 'endDate', value: '{{end_date}}', description: 'End date (ISO)' },
...
function createRequestObject(method: string, pathSegments: string[]): PostmanRequest {
  ...
  const request: PostmanRequest = {
    ...
    url: {
      raw: `{{base_url}}/api/${pathWithParams.join('/')}${
        queryParams.length ? '?' + queryParams.map(p => `${p.key}=${p.value}`).join('&') : ''
      }`,
      host: ['{{base_url}}'],
...
async function processDirectory(dirPath: string, basePath: string = ''): Promise<PostmanItem[]> {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      const subItems = await processDirectory(fullPath, path.join(basePath, entry));
      
      if (subItems.length > 0) {
        // Group similar endpoints together
        const groupName = folderName.charAt(0).toUpperCase() + folderName.slice(1);
