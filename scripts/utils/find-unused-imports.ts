  ...
  imports.forEach((importItem) => {
    if (importItem.type === "named") {
      importItem.imports.forEach((namedImport) => {
        ...
        const usageCount = (contentWithoutImports.match(usageRegex) || [])
          .length;
      ...
      importItem.type === "default" ||
      importItem.type === "namespace"
    ) {
      // Count occurrences outside of import statements
      const importRegex = new RegExp(`import`);
      const contentWithoutImports = content
...
if (results.unusedDefaultImports.length === 0) {
  ...
  results.unusedDefaultImports.forEach((item) => {
    ...
    console.log(
      colors.blue(`    Suggestion: Remove this import if not needed`),
    );
    console.log();
  });
}
...
if (results.unusedNamedImports.length === 0) {
  ...
  results.unusedNamedImports.forEach((item) => {
    ...
    console.log(colors.yellow(`    ${item.import.trim()}`));
    console.log(
      colors.blue(`    Suggestion: Remove "${item.name}" from the import`),
    );
    console.log();
  });
}
...
if (results.potentiallyUnused.length === 0) {
  ...
  results.potentiallyUnused.forEach((item) => {
    ...
    console.log(colors.yellow(`    ${item.import.trim()}`));
    console.log(
      colors.blue(`    Suggestion: Check if this import is actually used`),
    );
    console.log();
  });
}
...
if (results.typeOnlyImports.length === 0) {
  console.log(
    colors.green("  No type-only imports that need optimization found!"),
  );
} else {
  results.typeOnlyImports.forEach((item) => {
    ...
    console.log(
      colors.blue(
        `    Suggestion: Change to "import type { ${item.name} } from '${item.module}'"`,
      ),
    );
    console.log();
  });
...
console.log(
  `Potentially unused namespace imports: ${results.potentiallyUnused.length}`,
);
console.log(
  `Type-only imports that could be optimized: ${results.typeOnlyImports.length}`,
);
