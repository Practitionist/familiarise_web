...
function debugTimezoneConversion() {
  ...
  console.log(
    `  Start: ${dbStartTime.toISOString()} (${dbStartTime.getUTCHours()}:${dbStartTime.getUTCMinutes().toString().padStart(2, "0")} UTC)`,
  );
  console.log(
    `  End: ${dbEndTime.toISOString()} (${dbEndTime.getUTCHours()}:${dbEndTime.getUTCMinutes().toString().padStart(2, "0")} UTC)`,
  );
  ...
  console.log(
    `  Start: ${format(dbStartLocal, "yyyy-MM-dd HH:mm:ss zzz", { timeZone: timezone })}`,
  );
  console.log(
    `  End: ${format(dbEndLocal, "yyyy-MM-dd HH:mm:ss zzz", { timeZone: timezone })}`,
  );
  ...
  console.log(
    `  Extracted hours: ${wrongStartHour}:${wrongStartMinute.toString().padStart(2, "0")} - ${wrongEndHour}:${wrongEndMinute.toString().padStart(2, "0")}`,
  );
  console.log(
    `  Applied to ${format(currentDateTz, "yyyy-MM-dd")} in ${timezone}:`,
  );
  console.log(
    `  Start: ${wrongStartDateTime.toISOString()} → ${wrongStartUTC.toISOString()}`,
  );
  console.log(
    `  End: ${wrongEndDateTime.toISOString()} → ${wrongEndUTC.toISOString()}`,
  );
  console.log(
    `  API shows: ${format(wrongStartUTC, "hh:mm a", { timeZone: timezone })} - ${format(wrongEndUTC, "hh:mm a", { timeZone: timezone })}`,
  );

  // What the CORRECT processing should be
  console.log(`\n✅ CORRECT processing should be:`);
  console.log(
    `  The database times should be treated as timezone-aware, not UTC patterns`,
  );
  console.log(
    `  For recurring weekly slots, we should extract the LOCAL time pattern from the stored datetime`,
  );
  ...
  console.log(
    `  Extracted LOCAL pattern: ${correctStartHour}:${correctStartMinute.toString().padStart(2, "0")} - ${correctEndHour}:${correctEndMinute.toString().padStart(2, "0")}`,
  );
  ...
  console.log(
    `  Start: ${correctStartDateTime.toISOString()} → ${correctStartUTC.toISOString()}`,
  );
  console.log(
    `  End: ${correctEndDateTime.toISOString()} → ${correctEndUTC.toISOString()}`,
  );
  console.log(
    `  API should show: ${format(correctStartUTC, "hh:mm a", { timeZone: timezone })} - ${format(correctEndUTC, "hh:mm a", { timeZone: timezone })}`,
  );
}
