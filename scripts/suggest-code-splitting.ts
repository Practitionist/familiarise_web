...
console.log(
  "\x1b[1m\x1b[34m🔍 Analyzing codebase for code splitting opportunities...\x1b[0m",
);
...
sourceFiles.forEach((filePath) => {
  ...
  while ((match = componentRegex.exec(content)) !== null) {
    ...
    if (componentName) {
      ...
      const isBelowFold = BELOW_FOLD_KEYWORDS.some(
        (keyword) =>
...
if (results.largeComponents.length === 0) {
  ...
  results.largeComponents.forEach((component) => {
    ...
    console.log(
      colors.yellow(
        `    - ${component.lines} lines, ${component.imports} imports`,
      ),
    );
    console.log(
      colors.blue(
        `    Suggestion: Split this component into smaller parts or use dynamic imports`,
      ),
    );
  });
}
...
if (results.importHeavyComponents.length === 0) {
  ...
  results.importHeavyComponents.forEach((component) => {
    ...
    console.log(
      colors.gray(`    - Imports: ${component.importList.join(", ")}`),
    );
    console.log(
      colors.blue(`    Suggestion: Use dynamic imports for heavy dependencies`),
    );
  });
}
...
if (results.belowFoldComponents.length === 0) {
  ...
  results.belowFoldComponents.forEach((component) => {
    ...
    console.log(
      colors.blue(
        `    Suggestion: Load this component lazily with dynamic imports`,
      ),
    );
  });
}
...
if (results.routeComponents.length === 0) {
  ...
  results.routeComponents.forEach((component) => {
    ...
    console.log(
      colors.blue(
        `    Suggestion: Use Next.js built-in code splitting or dynamic imports`,
      ),
    );
  });
}
...
console.log(
  "2. Implement dynamic imports for below-the-fold and heavy components",
);
console.log("3. Run the bundle analyzer to see the impact of your changes");
console.log(
  "4. Measure performance improvements with Lighthouse or Core Web Vitals",
);
