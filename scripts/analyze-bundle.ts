...
try {
  if (!fs.existsSync(ANALYSIS_PATH)) {
    ...
    console.log(
      colors.yellow(
        "Run ANALYZE=true npm run build first to generate the analysis file.",
      ),
    );
    process.exit(1);
  }
  ...
  if (foundUiLibraries.length > 0) {
    console.log(
      colors.yellow("- Consider using tree-shaking for UI libraries:"),
      foundUiLibraries.join(", "),
    );
  }
  ...
  const largeUtils = sortedModules
    .filter(
      (mod) =>
        (mod.name.includes("lodash") ||
          mod.name.includes("moment") ||
          mod.name.includes("date-fns")) &&
        mod.size > 30000,
    )
    .map((mod) => mod.name);
  ...
  if (largeUtils.length > 0) {
    console.log(
      colors.yellow(
        "- Consider importing specific utilities instead of entire libraries:",
      ),
      largeUtils.join(", "),
    );
    if (largeUtils.some((name) => name.includes("moment"))) {
      console.log(
        "  → Replace moment.js with date-fns or day.js for smaller bundle size",
      );
    }
    if (largeUtils.some((name) => name.includes("lodash"))) {
      console.log(
        '  → Import specific lodash functions: import _map from "lodash/map"',
      );
    }
  }
  ...
  if (duplicates.length > 0) {
    ...
    duplicates.forEach((dup) => {
      console.log(
        `  → ${dup.name} (${dup.count} instances, total: ${formatSize(dup.totalSize)})`,
      );
    });
  }
  ...
  console.log(colors.bold.blue("\n=== NEXT STEPS ===\n"));
  console.log(
    "1. Use dynamic imports for large components not needed on initial load",
  );
  console.log(
    "2. Implement proper code-splitting with React.lazy() and Suspense",
  );
  console.log(
    "3. Consider using lightweight alternatives for large dependencies",
  );
  console.log(
    "4. Remove unused CSS with PurgeCSS or Tailwind's built-in purging",
  );
