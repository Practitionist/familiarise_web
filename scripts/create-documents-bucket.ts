...
async function createDocumentsBucket(): Promise<BucketCreationResult> {
  ...
  try {
    ...
    if (listError) {
      ...
      const existingBucket = existingBuckets?.find(bucket => bucket.name === 'documents');
      if (existingBucket) {
    ...
    const { data: buckets, error: verifyError } = await supabaseAdmin.storage.listBuckets();
    ...
    if (verifyError) {
      ...
      if (documentsBucket) {
        ...
        console.log('📁 Allowed file types: PDF, Word, Excel, Images, Text, etc.');
      } else {
