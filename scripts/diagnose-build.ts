...
console.log(`${colors.bold}${colors.blue}Next.js Build Diagnostics Tool${colors.reset}\n`);
console.log(`${colors.cyan}Analyzing your project for build issues...${colors.reset}\n`);
...
try {
  console.log(`${colors.cyan}Running Next.js build with verbose logging...${colors.reset}`);
  ...
  // Begin diagnostics
  console.log(`${colors.red}Build failed. Starting diagnostics...${colors.reset}\n`);
  ...
  const errorPatterns: ErrorPattern[] = [
    {
      ...
      solution: "Update your Node.js version to match the requirements for your Next.js version."
    },
    {
      pattern: /(.*) is defined but never used/,
      issue: "Unused Import/Variable",
      solution: "Remove unused imports or variables, or run \"npm run fix:imports\" to fix automatically."
    },
    {
      pattern: /Cannot find module '(.*)'/,
      issue: "Missing Module",
      solution: "Install the missing dependency or check for typos in import statements."
    },
    {
      pattern: /Type error:/,
      issue: "TypeScript Type Error",
      solution: "Fix the type issues in your code or add proper type definitions."
    },
    {
    ...
    },
    {
      ...
      solution: "Check the specific error messages above for details on what failed to compile."
    },
    {
      pattern: /Out of Memory/,
      issue: "Memory Limit Exceeded",
      solution: "Increase Node.js memory limit using NODE_OPTIONS=\"--max-old-space-size=4096\""
    },
    {
      pattern: /ENOSPC/,
      issue: "File Watching Limit Exceeded",
      solution: "Increase system file watching limits (common on Linux systems)."
    }
  ];
  ...
  const eslintErrors = errorLines.filter(line => line.includes("Error:") && line.match(/\d+:\d+/));
  if (eslintErrors.length > 0) {
    foundIssues.push({
      issue: "ESLint Errors",
      details: `${eslintErrors.length} ESLint errors found`,
      solution: "Run \"npm run lint:fix\" to attempt automatic fixes or modify your ESLint configuration."
    });
    
    diagnosticsReport.push("## ESLint Errors");
    diagnosticsReport.push(`Found ${eslintErrors.length} ESLint errors that need to be fixed.`);
    diagnosticsReport.push("Solution: Run \"npm run lint:fix\" or fix the errors manually.\n");
    ...
    if (eslintErrors.length > 10) {
      diagnosticsReport.push(`- ... and ${eslintErrors.length - 10} more errors`);
    }
  ...
  if (foundIssues.length === 0) {
    ...
    diagnosticsReport.push("1. Check the full build log for specific error messages");
    ...
    diagnosticsReport.push("5. Verify your environment variables are correctly set");
  }
  ...
  console.log(`${colors.bold}${colors.yellow}Diagnostic Results:${colors.reset}\n`);
  
  if (foundIssues.length > 0) {
    foundIssues.forEach(({ issue, details, solution }) => {
      console.log(`${colors.bold}${colors.red}Issue:${colors.reset} ${issue}`);
      if (details) console.log(`${colors.bold}Details:${colors.reset} ${details}`);
      console.log(`${colors.bold}${colors.green}Solution:${colors.reset} ${solution}\n`);
    });
  } else {
    console.log(`${colors.yellow}No common error patterns detected. Check the full logs for details.${colors.reset}\n`);
  }
  ...
  console.log(`${colors.cyan}For more help, review the logs or run with specific debugging flags:${colors.reset}`);
  console.log(`- DEBUG=* npm run build (for Next.js internal debugging)`);
  console.log(`- NODE_OPTIONS="--inspect" npm run build (for Node.js debugging)\n`);
